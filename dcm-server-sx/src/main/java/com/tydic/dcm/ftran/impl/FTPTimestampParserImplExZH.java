...
public class FTPTimestampParserImplExZH extends FTPTimestampParserImpl {
	...
	public Calendar parseTimestamp(String timestampStr) throws ParseException {
		...
		if (parsed != null && pp.getIndex() == timeStampStrPlusYear.length()) {
			...
			String year = Integer.toString(now.get(1));
			timeStampStrPlusYear = timestampStr + " " + year;
			SimpleDateFormat hackFormatter = new SimpleDateFormat(this.defaultDateFormat.toPattern() + " yyyy", this.defaultDateFormat.getDateFormatSymbols());
			hackFormatter.setLenient(false);
			hackFormatter.setTimeZone(this.defaultDateFormat.getTimeZone());
			parsed = hackFormatter.parse(timeStampStrPlusYear, pp);
