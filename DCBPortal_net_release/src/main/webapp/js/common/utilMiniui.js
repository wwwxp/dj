...
function closeWindow(data)
{
    ...
    }else if(window.frameElement.id && window.frameElement.id.indexOf(systemVar.IFRAME_PREFIX) != -1){
        close_jumpPage(data);
    }
    else{
        window.close();
 ...
 * @param _menuId  新窗口id
 * @param URL   路径
 * @param PRIVILEGE_NAME   窗口名称
 */
function open_dialog_jumpPage(_menuId, URL, PRIVILEGE_NAME,params) {
    var par_id = windowp.frameElement.id;
    var pars_id = par_id.substr(par_id.indexOf('_') + 1);
    var item = {
        "_menuId": pars_id + _menuId,
        "_close": 'true',
        "URL": URL,
    ...
    var iframDocument;
    if (windowp.jumpPage) {
        iframDocument = windowp.jumpPage(item);
        $("#close_" + pars_id + _menuId, windowp.document).addClass("close");
    }else {
        iframDocument = windowp.parent.jumpPage(item);
        $("#close_" + pars_id + _menuId, windowp.parent.document).addClass("close");
    }
    if(!iframDocument)return;
    iframDocument.onload = function(){
        this.contentWindow.onLoadComplete(params);;
    }
}
 ...
 * @param url     url
 * @param callback 回调函数
 * @param params   传递参数 自定义格式   (新窗口打开为对象,弹出框为自定义)
 * @param opt    选项，自定义扩展
 * @param _menuId  tab页面idhostList.html
 * @param flag  boolean 类型(false弹出框，true新窗口)
 */
function showDialogJumpPage(title,width,height,url,callback,params,opt,_menuId,flag){
    // 存在多级嵌套iframe
    if ((windowp.document.getElementById("tabMenu") && flag) || (windowp.parent.document.getElementById("tabMenu") && flag)){
        var tempUrl = url + "?menuId=" +_menuId;
        open_dialog_jumpPage(_menuId, tempUrl, title, params);
    }else {
        showDialog(title,width,height,url,callback,params,opt);
    }
}
...
/**
 * 显示成功级别的消息弹出框（非模式窗口，过几秒消息）
 * @param msg       消息内容 必输项
 * @param  callback 回调函数 可选项
 */
function showMessageTips(msg, callback) {
	var toastrObj = top.toastr;
	if (!toastrObj) {
        toastrObj = toastr;
	}
	//成功
    toastrObj.success(msg);
...
function showWarnMessageTips(msg, callback) {
    var toastrObj = top.toastr;
    if (!toastrObj) {
        toastrObj = toastr;
    }
    //成功
    toastrObj.warning(msg);
...
function showErrorMessageTips(msg, callback) {
    var toastrObj = top.toastr;
    if (!toastrObj) {
        toastrObj = toastr;
    }
 ...
 * @param  callback 回调函数 可选项
 */
function showErrorMessageAlterWithConfig(msg,callback, width, height) {
    var miniObject=top.mini;
     if(miniObject==null){
         miniObject = mini;
     }
 ...
 * @param params   传递参数 自定义格式
 * @param opt    选项，自定义扩展
 * @param _menuId  tab页面id
 * @param flag  boolean 类型(false弹出框，true新窗口)
 */
function showAddDialogJumpPage(title,width,height,url,callback,params,opt,_menuId,flag){
    if ((windowp.document.getElementById("tabMenu") && flag) || (windowp.parent.document.getElementById("tabMenu") && flag)){
        var tempUrl = url + "?menuId=" +_menuId;
        open_add_jumpPage(_menuId, tempUrl, title, params);
    }else {
        showAddDialog(title,width,height,url,callback,params,opt);
    }
}
 ...
 * @param flag  boolean 类型(false弹出框，true新窗口)
 */
function showEditDialogJumpPage(title,width,height,url,callback,params,opt,_menuId,flag){
    if ((windowp.document.getElementById("tabMenu") && flag) || (windowp.parent.document.getElementById("tabMenu") && flag)){
        var tempUrl = url + "?menuId=" +_menuId;
        open_edit_jumpPage(_menuId, tempUrl, title, params);
    }else {
        showEditDialog(title,width,height,url,callback,params,opt);
    }
}
 ...
 * @param params  参数
 */
function open_add_jumpPage(_menuId, URL, PRIVILEGE_NAME,params) {
    var par_id = window.frameElement.id;
    var pars_id = par_id.substr(par_id.indexOf('_') + 1);
    var item = {
        "_menuId": pars_id + _menuId,
        "_close": 'true',
    ...
    if(!iframDocument)return;
    iframDocument.onload = function(){
        this.contentWindow.onLoadComplete(systemVar.ADD,params);;
    }
}
 ...
 * @param params  参数
 */
function open_edit_jumpPage(_menuId, URL, PRIVILEGE_NAME,params) {
    var par_id = window.frameElement.id;
    var pars_id = par_id.substr(par_id.indexOf('_') + 1);
    var item = {
        "_menuId": pars_id + _menuId,
    ...
    var iframDocument = windowp.jumpPage(item);
    $("#close_" + pars_id + _menuId, windowp.document).addClass("close");
    if(!iframDocument)return;
    iframDocument.onload = function(){
        this.contentWindow.onLoadComplete(systemVar.EDIT,params);;
    }
}
 ...
 * @param data
 */
function close_jumpPage(data) {
    var _iframeId = window.frameElement.id;
    var _menuId = _iframeId.substring(_iframeId.indexOf(systemVar.IFRAME_PREFIX) + 16, _iframeId.length);
    var menuId = getQueryString('menuId');
    var par_menuId = _menuId.substring(0, _menuId.indexOf(menuId));
    var URL = windowp.document.getElementById("iframeContainer_"+par_menuId).src;;
    var item = {};
    if (par_menuId != undefined && par_menuId != '') {
        var PRIVILEGE_NAME = windowp.document.getElementById("tabMenu_"+par_menuId).text;
        item = {
            "_menuId": par_menuId,
            "URL": URL,
            "_close": 'true',
        ...
        windowp.jumpPage(item);
        var content = windowp.document.getElementById(systemVar.IFRAME_PREFIX + par_menuId);

        // 多层级iframe  &&data["IFRAME"]
        if(!isEmptyObject(data)){
            content = $(content).contents().find("iframe")[0].contentWindow;
            if(!content.reloadComplete)return;
            content.reloadComplete(data);
        }else {
            content.contentWindow.location.reload();
        }
    }
    windowp.closeCurrentTab(_menuId);
}
