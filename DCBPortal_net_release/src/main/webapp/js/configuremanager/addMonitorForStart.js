...
function addConfigRow(currentRow, hostId, hostIp, sshUser, isFull, fullObj, instId) {
	...
	var str = '<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'" data-HOST_IP="'+hostIp+'" data-HOST_ID="'+hostId+'" data-INST_ID="'+instId+'" >'
			...
			+'	data="getSysDictData(\'monitor_deploy_type\')" textField="text" valueField="code" value="monitor" required="true" '
			...
			+'	valueField="relativePath" textField="fileName" allowInput="false" parentField="parentRelativePath" '
			...
			+'			onclick="showStartOperationLog(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\', \'\', \'\', \''+timeSeq+'\')"><img src="../../..//images/comlog24.png" /></div>'
			+'	</div>'
			+'</td>'
...
function onSubmit(){
	...
	for (var i=0; i<params.length; i++) {
		if (params[i]["DEPLOY_TYPE"] == "" || params[i]["VERSION"] == "") {
            showWarnMessageTips("启动模式、版本不能为空，请选择！");
			return;
		}
	}
...
function postAjaxForDeploy(params){
	...
	getJsonDataByPost(Globals.baseActionUrl.MONITOR_TASK_ACTION_DEPLOY_INFOMATION_URL,params,"启停管理--启动Monitor",
		function(result){
...
function heightLightKeyWord(){
	...
	$("#deployTextarea").textSearch("failed,error,Not Exist",{textReduction:false,divStr:",",markCss: "font-weight:bold;color:#CF5130;letter-spacing:.1em;",nullReport:false,caseIgnore:true});
}
