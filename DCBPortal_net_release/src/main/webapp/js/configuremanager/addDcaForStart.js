...
function initStartVersion() {
	...
	//设置查询折叠查询条件
	mini.get("QR_VERSION").setData(versionList);
}
 ...
 * @param params
 */
function initQueryHostList(params) {
    comboxLoad(mini.get("HOST_ID"), params, "hostStart.queryStartHostList");
...
function search() {
	...
	var params = queryForm.getData(true);
    queryInstConfigList(params);
}

/**
 * 查询实例
 */
function queryInstConfigList(params) {
	if (params != null && typeof(params) != 'undefined') {
        params["CLUSTER_ID"] = param["CLUSTER_ID"];
        params["CLUSTER_TYPE"] = param["CLUSTER_TYPE"];
	} else {
        params = {
		...
		if(state==1){//数据库:运行
			if(result.processState==1){//远程主机：运行
				if(rowData.DEPLOY_FILE_TYPE == "m2db"){
                    showMessageTips(rowData.DEPLOY_FILE_TYPE+"程序正在运行，实例名为【"+processInfo+"】");
				}else{
                    showMessageTips(rowData.DEPLOY_FILE_TYPE+"程序正在运行，进程号为【"+processInfo+"】");
				}
	...
	if (selRows != null) {
		for (var i=0; i<selRows.length; i++) {
			...
			if (!isCreate && selRow["STATUS"] != busVar.STATE_ACTIVE) {
				selRow["CONFIG_PATH"] = selRow["INST_PATH"];
...
function singleChange(obj){
	...
	if (checked || checked == "checked") {
		...
		str+='<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'"  data-HOST_IP="'+hostIp+'" >'
			...
			+'	name="ip_'+timeSeq+'" id="ip_'+timeSeq+'" value="'+hostIp + '(' + sshUser + ')'+'" style="width:100%;"></td>'
			...
			+'	data="getSysDictData(\'dca_deploy_type\')" textField="text" valueField="code" required="true" '
			+'	showNullItem="false" style="width:100%;"/></td>'
			
			+'<th><span class="fred">*</span>配置文件：</th>'
			+'<td><input class="mini-treeselect" id="file_'+timeSeq+'" onvaluechanged="valueChange" popupHeight="220px" popupWidth="520px" '
			...
			+'	name="file_'+timeSeq+'" style="width:100%;"/></td>'
			...
			+'	name="version_'+timeSeq+'" style="width:100%;"/></td>'
			...
			+'<div style="text-align:left;width:45%;cursor:pointer;float:left;margin:2px 2px 2px 5px;" title="删除" id="removeRow_' + timeSeq + '" name="removeRow_'+timeSeq+'" href="javascript:void(0)" onclick="delCurrentRow(this, \''+timeSeq+'\', \''+hostIp+'\', \''+hostId+'\')"><img src="../../../images/deployHost/del_row.png" /></div></div>'
			+'</td>'
		...
		//获取Redis配置文件列表
		var redisData = [];
		for (var i=0; i<lastFileData.length; i++) {
			if (lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.REDIS) {
				redisData.push(lastFileData[i]);
			}
		}
		
		//获取全量刷新、增量刷新、全量校验、修复程序配置文件列表
		var redisRefreshData = [];
		for (var i=0; i<lastFileData.length; i++) {
			if (lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.REDIS_INC_REFRESH
					|| lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.REDIS_WHOLE_REFRESH
					|| lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.REDIS_WHOLE_CHECK
					|| lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.REDIS_INC_CHECK
					|| lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.REDIS_REVISE) {
				redisRefreshData.push(lastFileData[i]);
			}
		}
		...
		var monitorData = [];
		for (var i=0; i<lastFileData.length; i++) {
			if (lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.DAEMON) {
				daemonData.push(lastFileData[i]);
			}
            if (lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.MONITOR
				|| lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.DCAMONITOR
				|| lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.DCASWITCH) {
                monitorData.push(lastFileData[i]);
            }
		}
		...
		for (var j=0; j<tempData.length; j++) {
			...
			//对于Redis，不同主机可以使用相同的配置文件
			for (var k=0; k<redisData.length; k++) {
				if (tempData[j]["rootName"] == busVar.REDIS) {
					tempData[j]["IS_USED"] = "false";
					if (tempData[j]["relativePath"] == tempData[j]["rootName"] + "/" + redisData[k]["INST_PATH"]
						&& tempData[j]["rootName"] == redisData[k]["DEPLOY_FILE_TYPE"]
						&& hostId == redisData[k]["HOST_ID"]) {
						tempData[j]["IS_USED"] = "true";
						break;
					}
				}
			}
			
			//对于全量刷新、增量刷新、全量校验、修复程序，不同主机可以使用相同的配置文件
			for (var k=0; k<redisRefreshData.length; k++) {
				if (tempData[j]["rootName"] == busVar.REDIS_INC_REFRESH
						|| tempData[j]["rootName"] == busVar.REDIS_WHOLE_REFRESH
						|| tempData[j]["rootName"] == busVar.REDIS_WHOLE_CHECK
						|| tempData[j]["rootName"] == busVar.REDIS_INC_CHECK
						|| tempData[j]["rootName"] == busVar.REDIS_REVISE) {
					tempData[j]["IS_USED"] = "false";
					if (tempData[j]["relativePath"] == redisRefreshData[k]["INST_PATH"]
						&& tempData[j]["rootName"] == redisRefreshData[k]["DEPLOY_FILE_TYPE"]
						&& hostId == redisRefreshData[k]["HOST_ID"]) {
						tempData[j]["IS_USED"] = "true";
						break;
					}
				}
			...
			//对于Daemon，不同主机可以使用相同的配置文件
            if (tempData[j]["rootName"] == busVar.DAEMON
				|| tempData[j]["rootName"] == busVar.MONITOR
                || tempData[j]["rootName"] == busVar.DCAMONITOR
                || tempData[j]["rootName"] == busVar.DCASWITCH) {
                tempData[j]["IS_USED"] = "false";
            }
            // for (var k=0; k<daemonData.length; k++) {
				// if (tempData[j]["rootName"] == busVar.DAEMON) {
				// 	tempData[j]["IS_USED"] = "false";
				// 	if (tempData[j]["relativePath"] == tempData[j]["rootName"] + "/" + daemonData[k]["INST_PATH"]
				// 		&& tempData[j]["rootName"] == daemonData[k]["DEPLOY_FILE_TYPE"]
				// 		&& hostId == daemonData[k]["HOST_ID"]) {
				// 		tempData[j]["IS_USED"] = "true";
				// 		break;
				// 	}
				// }
            // }
            //
            // //对于monitor，不同主机可以使用相同的配置文件
            // for (var k=0; k<monitorData.length; k++) {
            //     if (tempData[j]["rootName"] == busVar.MONITOR) {
            //         tempData[j]["IS_USED"] = "false";
            //         if (tempData[j]["relativePath"] == tempData[j]["rootName"] + "/" + monitorData[k]["INST_PATH"]
            //             && tempData[j]["rootName"] == monitorData[k]["DEPLOY_FILE_TYPE"]
            //             && hostId == monitorData[k]["HOST_ID"]) {
            //             tempData[j]["IS_USED"] = "true";
            //             break;
            //         }
            //     }
		...
		mini.get(tid).tree.setExpandOnDblClick(false);
		mini.get(tid).tree.setExpandOnDblClick(false);
		//渲染下拉树控件图标
		mini.get(tid).tree.on("drawNode", function(e) {
            ...
            if (node["parentId"] == "" || node["parentId"] == null) {
            	e.nodeHtml += '<label style="cursor:pointer;margin-left:20px;text-decoration:underline;" id="selectAll_' + timeSeq + '" name="selectAll_' + timeSeq + '" onclick="changeNodeList(this, \'' + tid + '\', \'Y\')">全选</label>';
            	e.nodeHtml += '&nbsp;/&nbsp;<label style="cursor:pointer;text-decoration:underline;" id="selectNotAll_' + timeSeq + '" name="selectNotAll_' + timeSeq + '" onclick="changeNodeList(this, \'' + tid + '\', \'N\')">全不选</label>';
            }
            ...
            if (node["fileType"] == "F") {
            	...
            	e.nodeHtml += '&nbsp;&nbsp;<img src="'+Globals.ctx + '/images/goto.gif" '
            			   +  ' style="cursor:pointer;" title="查看配置文件" onclick="showConfig(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\', \''+instPath+'\', \''+fileName+'\')" />';

                e.nodeHtml += '&nbsp;&nbsp;<img src="'+Globals.ctx + '/images/comlog.png" '
                    +  ' style="cursor:pointer;" title="最近一次操作日志" onclick="showStartOperationLog(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\', \''+instPath+'\', \''+instPath+'\', \''+timeSeq+'\')" />';

            }
            //查看目录下的配置文件
            if ((node["rootName"] == "loadredis"
				|| node["rootName"] == busVar.DCAS
				|| node["rootName"] == busVar.DCAM
				|| node["rootName"] == busVar.DCAMONITOR
				|| node["rootName"] == busVar.DCASWITCH) && node.parentId != null && node.parentId != "") {
            	var hostId = node["HOST_ID"];
            	var instPath = node["relativePath"];
            	var fileName = node["fileName"];
            	e.nodeHtml += '&nbsp;&nbsp;<img src="'+Globals.ctx + '/images/goto.gif" '
            			   +  ' style="cursor:pointer;" title="查看配置文件" onclick="showMutilConfig(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\', \''+instPath+'\')" />';
 ...
 * 选中书目录所有节点
 */
function changeNodeList(obj, tid, selectType) {
	mini.get(tid).tree.expandAll();
	
	if (selectType == "Y") {
		var nodes = mini.get(tid).tree.findNodes(function(node){
			if (node["parentName"] != busVar.DEFAULT && node["IS_USED"] != "true") {
				if ((node["rootName"] == busVar.DCAM || node["rootName"] == busVar.DCAS
						|| node["rootName"] == busVar.DCAMONITOR || node["rootName"] == busVar.DCASWITCH)
						&& node["fileType"] == "D" && node["fileName"] != node["rootName"] && node["fileName"] != busVar.DEFAULT) {
					return true;
				} else if (node["fileType"] == "F" && node["fileName"] != node["rootName"]) {
					return true;
				}
			}
			return false;
		});
		mini.get(tid).tree.checkNodes(nodes);
		
		selNodeList = mini.get(tid).tree.getCheckedNodes(false);
		var nodeValues = [];
		for (var i=0; i<nodes.length; i++) {
			nodeValues.push(nodes[i]["relativePath"]);
		}
		mini.get(tid).setValue(nodeValues.join(","));
	} else if (selectType == "N") {
		mini.get(tid).tree.uncheckAllNodes();
		mini.get(tid).setValue('');
	}
	changeConfigTree(tid);
}
...
function addConfigRow(currentRow, hostId, hostIp, sshUser, isFull, fullObj, instId) {
	...
	str+='<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'" data-HOST_IP="'+hostIp+'" data-INST_ID="'+instId+'" >'
		...
		+'<th><span class="fred">*</span>配置文件：</th>'
		+'<td><input class="mini-treeselect" id="file_'+timeSeq+'" onvaluechanged="valueChange" popupHeight="220px" popupWidth="100%" '
		+'	valueField="relativePath" textField="fileName" allowInput="false" parentField="parentRelativePath"  showFolderCheckBox="false" showCheckBox="true" multiSelect="true" '
		+'	name="file_'+timeSeq+'" style="width:100%;"/></td>'
	...
	mini.get(tid).tree.on("drawNode", function(e) {
	    ...
	    //查看目录下的配置文件
	    if ((node["rootName"] == "loadredis" || node["rootName"] == "dcas" || node["rootName"] == "dcam") && node.parentId != null && node.parentId != "") {
	    	var hostId = node["HOST_ID"];
	    	var instPath = node["relativePath"];
	    	var fileName = node["fileName"];
	    	e.nodeHtml += '&nbsp;&nbsp;<img src="'+Globals.ctx + '/images/goto.gif" '
	...
	if (isFull) {
		...
		if (fullObj["DEPLOY_FILE_TYPE"] == busVar.REDIS) {
			//redis/192.168.161.27_8897/redis.conf
			var fileName = fullObj["CONFIG_PATH"].substring(fullObj["CONFIG_PATH"].lastIndexOf("/") + 1);
			var filePort = fullObj["CONFIG_PATH"].substring(fullObj["CONFIG_PATH"].indexOf("_") + 1, fullObj["CONFIG_PATH"].lastIndexOf("/"));
			var textFileName = fileName + "(" + filePort + ")";
			mini.get("file_" + timeSeq).setText(textFileName);
		}
...
function addOperator() {
	...
	if (rowData != null && rowData.length > 0) {
		for (var i=0; i<rowData.length; i++) {
			...
			if (deployFileType == "") {
                showWarnMessageTips("启动模式不能为空，请选择！");
				return;
			}
			
			if (version == "") {
                showWarnMessageTips("版本不能为空，请选择！");
				return;
			}
			
			if (configPath == "") {
                showWarnMessageTips("配置文件不能为空，请选择！");
				return;
			}
			...
			for (var j=0; j<rowData.length; j++) {
				...
				if (i != j && deployFileType != busVar.DAEMON && (deployFileType == nextDeployFileType && configPath == nextConfigPath)) {
...
function refreshData() {
	...
	$("#paramsTable>tbody:eq(0)>tr").each(function(index, item){
		...
		for (var j=0; j<tempData.length; j++) {
			...
			//获取到最根节点
			if ((tempData[j]["parentId"] == "" || tempData[j]["parentId"] == null)
				&& (tempData[j]["parentRelativePath"] == null || tempData[j]["parentRelativePath"] == "")) {
				rootData.add(tempData[j]);
			}
		}
...
function changeDeploy(id, index) {
	...
	for (var i=0; i<tempData.length; i++) {
		if (tempData[i]["rootName"] == deployType) {
			if (deployType == busVar.REDIS && (tempData[i]["fileName"] == "default" || tempData[i]["relativePath"] == "redis/default/redis.conf")) {
				continue;
			}

            if (deployType == busVar.SENTINEL && (tempData[i]["fileName"] == "default" || tempData[i]["relativePath"] == "sentinel/default/sentinel.conf")) {
                continue;
            }

            if (deployType == busVar.DCASWITCH && (tempData[i]["fileName"] == "default" || tempData[i]["relativePath"] == "switchDCA/default/config.js")) {
                continue;
            }

            if (deployType == busVar.DCAMONITOR && (tempData[i]["fileName"] == "default" || tempData[i]["relativePath"] == "monitorDCA/default/config.js")) {
                continue;
            }
...
function valueChange(e) {
	var objId = e.source.id;
	changeConfigTree(objId);
}
 ...
 * @param treeObjId
 */
function changeConfigTree(treeObjId) {
	var selNodeList = mini.get(treeObjId).tree.getCheckedNodes();
	if (selNodeList != null && selNodeList.length > 0) {
		var totalFileName = [];
		for (var i=0; i<selNodeList.length; i++) {
			var nodeUsed = selNodeList[i]["IS_USED"];
			//选择Redis节点的时候需要在文本框显示对应的端口
			if ((selNodeList[i]["rootName"] == busVar.REDIS || selNodeList[i]["rootName"] == busVar.SENTINEL) && selNodeList[i]["fileType"] == "F") {
				var fileName = selNodeList[i]["fileName"];
				var parentFileName = selNodeList[i]["parentRelativePath"];
				var filePort = parentFileName.substring(parentFileName.indexOf("_") + 1);
				var textFileName = fileName + "(" + filePort + ")";
				totalFileName.push(textFileName);
			} else if ((selNodeList[i]["rootName"] == busVar.DCASWITCH || selNodeList[i]["rootName"] == busVar.DCAMONITOR) && selNodeList[i]["fileType"] == "F") {
                var fileName = selNodeList[i]["fileName"];
                var parentFileName = selNodeList[i]["parentRelativePath"];
                var instPath = "";
                if (!isNull(parentFileName) && parentFileName.indexOf("/") > 0) {
                    instPath = parentFileName.substring(parentFileName.indexOf("/") + 1, parentFileName.length) + "/"
				}
                var textFileName = instPath + fileName;
                totalFileName.push(textFileName);
			} else {
				//选中节点为已经使用或者为非叶子节点不能被选中
				if (nodeUsed == "true" || selNodeList[i]["parentRelativePath"] == null
					|| (selNodeList[i]["fileType"] == "D" && (selNodeList[i]["rootName"] == busVar.REDIS|| selNodeList[i]["rootName"] == busVar.SENTINEL))) {
					mini.get(treeObjId).tree.disableNode(selNodeList[i]);
					mini.get(treeObjId).tree.uncheckNode(selNodeList[i]);
				} else {
					totalFileName.push(selNodeList[i]["fileName"]);
				}
			}
		}
		mini.get(treeObjId).setText(totalFileName.join(","));
	}
}
...
function onSubmit(){
	...
	$("#paramsTable>tbody:eq(0)>tr").each(function(index, item){
		...
		var fileTextStr = mini.get("file_"+time).getText();
		if (fileStr != null) {
			var fileList = fileStr.split(",");
			var fileTextList = fileTextStr.split(",");
			for (var i=0; i<fileList.length; i++) {
				var fileText = fileTextList[i];
				var file = fileList[i];
				//Redis和 sentienl启动端口
				var filePort = "";
				if (fileText != null && fileText.indexOf("(") != -1 && fileText.indexOf(")") != -1) {
					filePort = fileText.substring(fileText.indexOf("(") + 1, fileText.indexOf(")"));
				}
	...
	for (var i=0; i<params.length; i++) {
		...
		for (var j=0; j<params.length; j++) {
			if (i != j && params[i]["DEPLOY_TYPE"] != busVar.DAEMON
				&& params[i]["DEPLOY_TYPE"] != busVar.MONITOR
                && params[i]["DEPLOY_TYPE"] != busVar.DCAMONITOR
            	&& params[i]["DEPLOY_TYPE"] != busVar.DCASWITCH
				&& (params[i]["DEPLOY_TYPE"] == params[j]["DEPLOY_TYPE"]
				&& params[i]["FILE"] == params[j]["FILE"])) {
				var hostIp = params[i]["HOST_IP"];
				var nextHostIp = params[j]["HOST_IP"];
				showWarnMessageAlter("相同启动模式不能选择同一配置文件,<br/>主机IP【"+hostIp + ", " + nextHostIp +"】,启动模式为【"+params[i]["DEPLOY_TYPE"]+"】， 配置文件【"+params[i]["FILE_TEXT"]+"】,请选择！");
				//showWarnMessageAlter("相同启动模式配置文件不能相同，请选择！");
				return;
			}
		}
	}
...
function postAjaxForDeploy(params){
	...
	getJsonDataByPost(Globals.baseActionUrl.DCA_TASK_ACTION_DEPLOY_INFOMATION_URL,params,"启停管理--启动dca",
			function(result){
	...
	getJsonDataByPost(Globals.baseActionUrl.DEPLOY_TASK_ACTION_GET_CONFIG_LIST_URL, param, "启停管理--获取启停配置文件",
		...
		if (result != null && result.length > 0) {
			...
			for (var i=0; i<JsVar["fileData"].length; i++) {
				...
				if ((JsVar["fileData"][i]["rootName"] == busVar.DCAM
						|| JsVar["fileData"][i]["rootName"] == busVar.DCAS
                    	|| JsVar["fileData"][i]["rootName"] == busVar.DCASWITCH
                    	|| JsVar["fileData"][i]["rootName"] == busVar.DCAMONITOR) && JsVar["fileData"][i]["fileType"] == "F") {
