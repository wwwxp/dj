...
function search() {
	...
	//集群ID
	params["QUERY_HOST_ID"] = mini.get("QUERY_HOST_ID").getValue();
	//程序状态
...
function load(params){
	...
	getJsonDataByPost(Globals.baseActionUrl.BUS_PROGRAM_LIST_ACTION_MANAGE_URL, params, "周边管理-获取周边程序列表",
...
function addProgram() {
    ...
    getJsonDataByPost(Globals.baseActionUrl.BUS_PROGRAM_TASK_ADD_ACTION_MANAGE_URL, programForm, "周边程序管理-添加周边程序实例",
		function success(result){
...
function checkHostState(index){
	...
	getJsonDataByPost(Globals.baseActionUrl.OTHER_CHECK_ACTION_MANAGE_URL, obj, "启停管理-检查周边程序状态",
		function(result){
			if(result["state"] == "1"){
				if(result["info"] != null){
...
function submit(index){
	...
	showConfirmMessageAlter("确定启动运行该程序实例吗？",function ok(){
		...
		getJsonDataByPost(Globals.baseActionUrl.OTHER_RUN_ACTION_MANAGE_URL, params, "启停管理-运行route程序",
	        function(result){
				if (result != null) {
					...
					if (result["flag"] == "error") {
						showErrorMessageAlter("程序实例启动失败，请检查！");
					} else {
                        showMessageTips("程序实例启动成功！");
						load({});
					}
				}
	        });
...
function stop(index){
	...
	showConfirmMessageAlter("确定停止运行该程序实例吗？",function ok(){
		...
		getJsonDataByPost(Globals.baseActionUrl.OTHER_STOP_ACTION_MANAGE_URL, params, "启停管理-停止route程序",
	        function(result){
				if (result != null) {
					var textValue=result.info + "<br/>返回结果:"+result.reason+"<br/>";
					textValue=textValue.replaceAll("\n","<br/>");
					...
					if (result["flag"] == "error") {
						showErrorMessageAlter("程序实例停止失败，请检查！");
					} else {
                        showMessageTips("程序实例停止成功！");
						load({});
					}
				}
		    });
