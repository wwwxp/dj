...
//加载表格
function load(param){
	getJsonDataByPost(Globals.baseActionUrl.BUS_PROGRAM_LIST_WITHOUT_HOST_ACTION_MANAGE_URL, param, "rent管理-获取rent程序列表",
...
function onActionRenderer(e) {
    ...
    html += '<a class="Delete_Button" href="javascript:scanDefinition(' + index + ')">查看定义</a>';
    return html;
}
...
function scanDefinition(index){
	...
	showDialog("启停管理-查看rent定义",900,550,Globals.baseJspUrl.RENT_VIEW_JSP_FILE_CONF_URL,
...
function submit(index){
    ...
    showConfirmMessageAlter("确定启动运行该程序吗？",function ok(){
		getJsonDataByPost(Globals.baseActionUrl.RENT_RUN_TASK_ACTION_MANAGE_URL,rowInfo,"启停管理-运行rent程序",
	        function(result){
				if (result != null) {
					var rstInfo = result.info.replaceAll("\n", "<br/>")+"<br/>返回信息："+result.reason.replaceAll("\n", "<br/>");
					$("#businessTextarea").html(rstInfo);
...
function stop(index){
    ...
    showConfirmMessageAlter("确定停止运行该程序吗？",function ok(){
		getJsonDataByPost(Globals.baseActionUrl.RENT_STOP_TASK_ACTION_MANAGE_URL,rowInfo,"启停管理-停止rent程序",
	        function(result){
				if (result != null) {
					var rstInfo = result.info.replaceAll("\n", "<br/>")+"<br/>返回信息："+result.reason.replaceAll("\n", "<br/>");
					$("#businessTextarea").html(rstInfo);
