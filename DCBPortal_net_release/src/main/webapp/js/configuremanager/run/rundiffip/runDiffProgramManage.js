...
function beforenodeselect(e) {
    var clusterType = JsVar["data"]["CLUSTER_TYPE"];
	 JsVar["check_isleaf"] = e.isLeaf;
    //禁止选中父节点
    if (e.isLeaf == true  && clusterType == 'dcm') e.cancel = true;
}
...
function load(params){
    ...
    getJsonDataByPost(Globals.baseActionUrl.BUS_PROGRAM_LIST_ACTION_MANAGE_URL, params, "程序启停管理-查询分IP启停程序列表",
        function success(result){
            if (result != null) {
...
function onActionRenderer(e) {
    ...
    var HOST_ID = (e.record.HOST_ID == null ||e.record.HOST_ID == "" || e.record.HOST_ID == undefined) ? JsVar["data"]["HOST_ID"] : e.record.HOST_ID
...
function loadConfigList() {
    ...
    getJsonDataByPost(Globals.baseActionUrl.BUS_PROGRAM_FILES_ACTION_MANAGE_URL, params, "程序启停管理-获取分IP启停程序配置文件",
        function success(result){
            if (result != null) {
...
function copyRow(index){
	...
	showAddDialog("业务程序操作",700,450,Globals.baseJspUrl.RUN_DIFF_IP_ADD_JSP_MANAGE_URL,
	        function destroy(data){
...
function addProgram() {
    ...
    getJsonDataByPost(Globals.baseActionUrl.BUS_PROGRAM_TASK_ADD_ACTION_MANAGE_URL, programForm, "程序启停管理-添加分IP启停程序实例",
        function success(result){
...
function delProgramTask(index) {
    ...
    showConfirmMessageAlter("确定删除所有版本该实例？",function ok(){
        ...
        getJsonDataByPost(Globals.baseActionUrl.BUS_PROGRAM_TASK_DEL_ACTION_MANAGE_URL, paramList, "程序启停管理-删除分IP启停程序实例",
            function(result){
	        	if(result){
					var message='';
...
function checkHostState(index){
    ...
    //$("#deployTextarea").html("");
    getJsonDataByPost(Globals.baseActionUrl.COMMON_DIFF_IP_CHECK_ACTION_MANAGE_URL, paramList, "程序启停管理-检查分IP启停程序实例状态",
        function(dataList){
	    	 if(index == undefined){
		    	 search(true);
...
function submit(index){
    ...
    if (rows.length > 0) {
        for (var i = 0; i < rows.length; i++) {
            if(rows[i]["RUN_STATE"] == 0 || rows[i]["RUN_STATE"] == null){
                ...
                rows[i]["HOST_ID"] = JsVar["data"]["HOST_ID"];
    ...
    showConfirmMessageAlter("确定启动运行该程序吗？",function ok(){
        ...
        $("#deployTextarea").html("");
        getJsonDataByPost(Globals.baseActionUrl.COMMON_DIFF_IP_RUN_ACTION_MANAGE_URL, params, "程序启停管理-启动分IP启停程序实例",
            function(result){
                if (result != null && result["RET_CODE"] == busVar.SUCCESS) {
                    ...
                    showErrorMessageTips("业务程序启动失败，请检查数据!");
                }
            });
    });
}
...
function stop(index){
    ...
    showConfirmMessageAlter("确定停止运行该程序吗？",function ok(){
        ...
        $("#deployTextarea").html("");
        getJsonDataByPost(Globals.baseActionUrl.COMMON_DIFF_IP_STOP_ACTION_MANAGE_URL, params, "程序启停管理-停止分IP启停程序实例",
            function(result){
                if (result != null && result["RET_CODE"] == busVar.SUCCESS) {
                    //停止程序实例个数
                    var totalCnt = result["TOTAL_CNT"];
                    ...
                    showErrorMessageTips("业务程序停止失败，请检查数据!");
                }
            });
    });
}
