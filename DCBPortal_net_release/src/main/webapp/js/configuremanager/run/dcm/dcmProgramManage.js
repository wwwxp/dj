...
function load(params){
	getJsonDataByPost(Globals.baseActionUrl.BUS_PROGRAM_LIST_ACTION_MANAGE_URL, params, "采集管理-获取采集程序列表",
...
function checkHostState(index){
	...
	$("#deployTextarea").html("");
	getJsonDataByPost(Globals.baseActionUrl.DCM_CHECK_ACTION_MANAGE_URL, obj, "启停管理-检查route程序状态",
	    function(result){
			if(result["state"] == "1"){
				if(result["info"] != null){
                    showMessageTips(result["info"]);
				}else{
                    showMessageTips("当前程序正在运行，进程号为【 " + result["process"] + "】");
				}
...
function submit(index){
	...
	showConfirmMessageAlter("确定启动运行该程序实例吗？",function ok(){
		...
		$("#deployTextarea").html("");
		getJsonDataByPost(Globals.baseActionUrl.DCM_RUN_ACTION_MANAGE_URL, params, "启停管理-运行route程序",
	        function(result){
				if (result != null) {
...
function stop(index){
	...
	showConfirmMessageAlter("确定停止运行该程序实例吗？",function ok(){
		...
		$("#deployTextarea").html("");
		getJsonDataByPost(Globals.baseActionUrl.DCM_STOP_ACTION_MANAGE_URL, params, "启停管理-停止route程序",
	        function(result){
				if (result != null) {
					var textValue=result.info + "<br/>返回结果:"+result.reason+"<br/>";
					textValue=textValue.replaceAll("\n","<br/>");
