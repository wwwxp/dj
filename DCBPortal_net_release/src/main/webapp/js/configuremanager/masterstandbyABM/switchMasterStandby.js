...
function updateGrayConfig(index) {
	...
	showDialog("主备切换--灰度配置文件修改",700, 260,Globals.baseJspUrl.SWITCH_ABM_PROGRAM_UPGRADE_CONFIG_JSP_MANAGE_URL,
...
function greyUpgrade(index){
	...
	gridParam.CLUSTER_TYPE = rowInfo.CLUSTER_TYPE;
	//程序编码
	//gridParam.PROGRAM_CODE = rowInfo.PROGRAM_CODE;
	//集群编码
	gridParam.CLUSTER_CODE = rowInfo.CLUSTER_CODE;
	//业务包名称
	gridParam.PACKAGE_NAME = rowInfo.PACKAGE_NAME;
	...
	showDialog("主备切换--灰度升级",700, 350,Globals.baseJspUrl.SWITCH_ABM_PROGRAM_UPGRADE_JSP_MANAGE_URL,
	    function destroy(data){
			var params = {
				BUS_CLUSTER_ID:JsVar["BUS_CLUSTER_ID"],
				RUN_STATE:"1"
...
function officialLaunch(index){
	...
	showConfirmMessageAlter("确定开始正式发布？",function ok(){
		getJsonDataByPost(Globals.baseActionUrl.SWITCH_ABM_MASTER_STANDBY_OPT_LAUNCH_ACTION_MANAGE_URL,gridParam,"主备切换-灰度升级",
			function(result){
				if (result.rstCode == "0") {
...
function rollback(index, actionFlag){
	...
	gridParam.PROGRAM_CODE_1=rowInfo.PROGRAM_CODE;
	showDialog("主备切换--回退",700, 360,Globals.baseJspUrl.SWITCH_ABM_PROGRAM_ROLLBACK_JSP_MANAGE_URL,
	    function destroy(data){
			var params = {
				BUS_CLUSTER_ID:JsVar["BUS_CLUSTER_ID"],
				RUN_STATE:"1"
