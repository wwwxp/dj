...
function onLoadComplete(data) {
	...
	var params = {
		...
		FILE_PATH:JsVar["DATA"]["filePath"]
	};
	getJsonDataByPost(Globals.baseActionUrl.CONFIGURE_ACTION_LOAD_FILE_DOWN_FOLDER_URL,params,"启停管理-查看配置文件",
			function(result){
			if(result != null && result.length > 0){
				JsVar["configFileCombo"].setData(result);
...
function loadContent(e){
	...
	params.filePath=filePath+"/"+fileName;
	params.HOST_ID=JsVar["DATA"]["HOST_ID"];
	getJsonDataByPost(Globals.baseActionUrl.CONFIGURE_ACTION_SHOW_CONFIG_FILE_CONTENT_URL,params,"配置修改-FTP/SFTP获取文件内容",
			function(result){
		if(result!=null){
