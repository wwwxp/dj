...
//初始化主机div
function queryHostDiv(checkHostId){
	...
	getJsonDataByPost(Globals.baseActionUrl.FRAME_QUERY_FOR_LIST_URL, params, "M2DB数据导入-主机查询",
			function(result){
				var str = "";
...
function chooseToStart(){
	...
	if(host_arry.length>0){
		var info = {};
		var b = new Base64();
		var encodeStr = b.encode(JSON.stringify(host_arry));
		info["hostList"] = encodeStr;
		...
		showConfirmMessageAlter("确定该操作？",function ok(){
			param["sumbitButton"].setText("正在导入");
			...
			commonFileUpload(Globals.baseActionUrl.HOSTSTART_ACTION_M2DB_INPUT_URL,['uFile'],info,
		 			function (result){
						//刷新输出信息
