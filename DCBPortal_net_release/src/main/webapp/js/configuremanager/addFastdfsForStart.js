...
function singleChange(obj){
	...
	if (checked || checked == "checked") {
		...
		str+='<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'" data-HOST_IP="'+hostIp+'"><th><span>主机：</span></th>'
			...
			+'<td><input class="mini-combobox" name="deployType_'+timeSeq+'" id="deployType_'+timeSeq+'" popupWidth="100%" '
			+'	data="getSysDictData(\'fastdfs_deploy_type\')" textField="text" valueField="code" required="true" '
			...
			+'<td><input class="mini-hidden" '
			+'	id="hostId_' + timeSeq + '" name="hostId_'+timeSeq+'" value="'+hostId+'"/></td> '
			+'</tr>';
		...
		for (var j=0; j<tempData.length; j++) {
			...
			//根据主机IP过滤配置文件
			if (tempData[j]["fileName"].indexOf("_") != -1 && tempData[j]["parentId"] == "") {
				if (tempData[j]["fileName"].indexOf(hostIp+"_") == -1) {
		...
		mini.get(tid).tree.on("drawNode", function(e) {
            ...
            if (node["fileType"] == "F") {
                ...
                e.nodeHtml += '&nbsp;&nbsp;<img src="'+Globals.ctx + '/images/comlog.png" '
                    +  ' style="cursor:pointer;" title="最近一次操作日志" onclick="showStartOperationLog(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\', \''+instPath+'\', \''+fileName+'\', \''+timeSeq+'\')" />';

            }
            
            //所有子节点加上超链接
        	if (node["fileType"] == "F" && node["IS_USED"] == "true") {
 ...
 * 查看程序启停日志信息
 */
function showOperationLog(clusterId, clusterType, hostId, instPath, fileName, timeSeq) {
    var params = {
        ...
        INST_PATH:instPath,
        FILE_NAME:instPath,
        DEPLOY_FILE_TYPE:mini.get("deployType_"+timeSeq).getValue(),
...
function addConfigRow(currentRow, hostId, hostIp, sshUser, isFull, fullObj, instId) {
	...
	str+='<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'" data-HOST_IP="'+hostIp+'" data-INST_ID="'+instId+'" >'
		...
		+'<td><input class="mini-treeselect" id="file_'+timeSeq+'" onvaluechanged="valueChange" popupHeight="220px" popupWidth="100%" '
		+'	valueField="relativePath" textField="fileName" allowInput="false" parentField="parentRelativePath" showFolderCheckBox="false" showCheckBox="true" multiSelect="true" '
		+'	name="file_'+timeSeq+'" style="width:100%;" required="true"/></td>'
		
		+'<th><span class="fred">*</span>启动版本：</th>'
		+'<td><input class="mini-combobox" id="version_'+timeSeq+'" popupWidth="100%" '
		+'	valueField="VERSION" textField="VERSION" allowInput="false"  data="getStartVersionData(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\')" '
		+'	name="version_'+timeSeq+'" style="width:100%;" required="true" /></td>'
	...
	for (var j=0; j<tempData.length; j++) {
		...
		//根据主机IP过滤配置文件
		if (tempData[j]["fileName"].indexOf("_") != -1 
				&& tempData[j]["parentId"] == "" 
...
function addOperator() {
	...
	for (var i=0; i<rowData.length; i++) {
		...
		if (deployFileType == "" || configPath == "" || version == "") {
            showWarnMessageTips("启动模式、配置文件、版本均不能为空，请选择！");
			return;
		}
			...
			if (result != null && result.length > 0 && JsVar["fileData"].length > 0) {
				for (var i=0; i<JsVar["fileData"].length; i++) {
					for (var j=0; j<result.length; j++) {
						if (JsVar["fileData"][i]["relativePath"] == result[j]["INST_PATH"]) {
...
function valueChange(e) {
	...
	if (selNodeList != null && selNodeList.length > 0) {
		...
		for (var i=0; i<selNodeList.length; i++) {
			...
			if (nodeUsed == "true" || selNodeList[i]["fileType"] == "D") {
...
function onSubmit(){
	...
	for (var i=0; i<params.length; i++) {
		...
		for (var j=0; j<params.length; j++) {
			if (i != j && (params[i]["DEPLOY_TYPE"] == params[j]["DEPLOY_TYPE"] && params[i]["FILE"] == params[j]["FILE"])) {
				var configFile = params[i]["FILE"].substr(params[i]["FILE"].indexOf("/")+1);
				showWarnMessageAlter("相同启动模式不能选择同一配置文件,<br/>主机IP【"+params[i]["HOST_IP"] + ", " + params[j]["HOST_IP"] +"】,启动模式为【"+params[i]["DEPLOY_TYPE"]+"】， 配置文件【"+configFile+"】,请选择！");
				return;
			}
		}
	}
	...
	postAjaxForDeploy(params, checkedHostLength);
}
 ...
 * @param params
 */
function postAjaxForDeploy(params, checkedHostLength){
	textValue +="主机正在启动...<br/>";
	...
	getJsonDataByPost(Globals.baseActionUrl.FASTDFS_TASK_ACTION_DEPLOY_INFOMATION_URL,params,"启停管理--启动Fastdfs",
		function(result){
			if(result && result["RST_STR"] != ""){
				textValue += result["RST_STR"];
				textValue=textValue.replaceAll("\n","<br/>");
				var endTimes = (new Date()).getTime();
	        	textValue += "启动结束时间: " + ((new Date(endTimes)).format("yyyy-MM-dd hh:mm:ss")) + ", 本次启动"+checkedHostLength+"个实例, 总耗时: " + ((endTimes - startTimes)/1000).toFixed(2) + "秒";
	        	$("#deployTextarea").html(textValue);
