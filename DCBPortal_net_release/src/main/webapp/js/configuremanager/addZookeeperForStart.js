...
function addConfigRow(currentRow, hostId, hostIp, sshUser, isFull, fullObj, instId) {
	...
	var str='<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'" data-HOST_IP="'+hostIp+'" data-HOST_ID="'+hostId+'" data-INST_ID="'+instId+'" >'
        ...
        +'		<div style="text-align:center;width:100%;cursor:pointer;" title="最近一次操作日志" '
		+'			id="log_' + timeSeq + '" name="log_'+timeSeq+'" href="javascript:void(0)" '
		+'			onclick="showStartOperationLog(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\', \'zoo.cfg\', \'zoo.cfg\', \''+timeSeq+'\')"><img src="../../..//images/comlog24.png" /></div>'
        +'	</div>'
        +'</td>'
...
function postAjaxForDeploy(params){
	...
	getJsonDataByPost(Globals.baseActionUrl.ZOOKEEPER_TASK_ACTION_DEPLOY_INFOMATION_URL,params,"启停管理--启动zookeeper",
			function(result){
