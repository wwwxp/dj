 ...
 *获取sp_switch.xml相关信息
 */
function getSpSwitchInfo(){
	var params = {
		PROGRAM_CODE:JsVar["DATA"]["PROGRAM_CODE"],
		PACKAGE_TYPE:JsVar["DATA"]["PACKAGE_TYPE"],
		//程序所属组 
		PROGRAM_GROUP:JsVar["DATA"]["PROGRAM_GROUP"],
		//业务包类型
...
function submitUpgrade(){
	...
	//提交到后台处理
	getJsonDataByPost(Globals.baseActionUrl.SWITCH_MASTER_STANDBY_OPT_UPGRADE_CONFIG_ACTION_MANAGE_URL,upgradeParams,"主备切换-灰度配置文件修改",
		function(result){
			if (result.rstCode == "0") {
