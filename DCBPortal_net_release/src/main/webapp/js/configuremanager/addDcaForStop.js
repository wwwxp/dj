...
function showConfigContent(index) {
	...
	if(deployType == busVar.DCA 
			...
			&& deployFileType != busVar.DCASWITCH
			&& deployFileType != busVar.DCAMONITOR) {   //多个配置文件
		var params = {
			INST_ID:instId,
			HOST_ID:hostId,
			filePath:filePath
...
function onSubmit() {
	...
	for (var i=0; i<stopArray.length; i++) {
		hostArray.push({
			...
			PORT:stopArray[i].PORT,
			DAEMON_STATE:stopArray[i].DAEMON_STATE,
			PM2_NAME:stopArray[i].PM2_NAME,
			CLUSTER_ID: param["CLUSTER_ID"],
...
function postAjaxForDeploy(host_arry){
	...
	getJsonDataByPost(Globals.baseActionUrl.DCA_TASK_ACTION_STOP_DEPLOY_INFOMATION_URL,host_arry,"启停管理--停止DCA",
		function(result){
			if(result && result["RST_STR"] != ""){
				textValue+=result["RST_STR"];
				textValue=textValue.replaceAll("\n","<br/>");
				var endTimes = (new Date()).getTime();
	        	textValue += "停止结束时间: " + ((new Date(endTimes)).format("yyyy-MM-dd hh:mm:ss")) + ", 本次停止"+host_arry.length+"个实例, 总耗时: " + Math.floor((endTimes - startTimes)/1000).toFixed(2) + "秒";
	        	$("#deployTextarea").html(textValue);
