...
function singleChange(obj) {
	...
	if (checked || checked == "checked") {
		...
		str+='<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'" data-HOST_IP="'+hostIp+'"><th><span>主机：</span></th>'
			...
			+'<td>'
			+'	<input class="mini-combobox" id="deployType_'+timeSeq+'" name="deployType_'+timeSeq+'" popupWidth="100%" '
			+'		onvaluechanged="changeDeploy(\'deployType_'+timeSeq+'\', '+timeSeq+')" '
			+'		data="getSysDictData(\'rocketmq_deploy_type\')" value="1" textField="text" valueField="code" required="true" '
			...
			+'<td>'
			+'	<input class="mini-hidden" id="hostId_'+timeSeq+'" name="hostId_'+timeSeq+'" value="'+hostId+'"/>'
			+'</td> '
			+'</tr>';
		...
		for (var i=0; i<lastFileData.length; i++) {
			if (lastFileData[i]["DEPLOY_FILE_TYPE"] == busVar.NAMESRV) {
				namesrvData.push(lastFileData[i]);
			}
		}
		...
		for (var j=0; j<tempData.length; j++) {
			...
			//对于Namesrv不同主机可以使用相同的配置文件
			for (var k=0; k<namesrvData.length; k++) {
				if (tempData[j]["rootName"] == busVar.NAMESRV) {
					tempData[j]["IS_USED"] = "false";
					if (tempData[j]["relativePath"] == namesrvData[k]["INST_PATH"]
						&& tempData[j]["rootName"] == namesrvData[k]["DEPLOY_FILE_TYPE"]
						&& hostId == namesrvData[k]["HOST_ID"]) {
...
function addConfigRow(currentRow, hostId, hostIp, sshUser, isFull, fullObj, instId) {
	...
	str+='<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'" data-HOST_IP="'+hostIp+'" data-INST_ID="'+instId+'">'
		...
		+'<td>'
		+'	<input class="mini-hidden" id="hostId_'+timeSeq+'" name="hostId_'+timeSeq+'" value="'+hostId+'"/></td> '
		+'</tr>';
	...
	if (isFull) {
		...
		//展开当前节点目录
		mini.get("file_" + timeSeq).tree.expandPath(currentNode);
		//选中节点
...
//	//选中节点为已经使用或者为非叶子节点不能被选中
//	if (isUsed == "true" || e.source.tree.getSelectedNode().fileType == "D") {
...
function onSubmit(){
	...
	for (var i=0; i<params.length; i++) {
		...
		for (var j=0; j<params.length; j++) {
			if (i != j && params[i]["DEPLOY_TYPE"] != busVar.NAMESRV && params[i]["FILE"] == params[j]["FILE"]) {
                showWarnMessageTips("broker启动模式下，配置文件不能相同，请选择！");
				return;
			}
		}
	}
...
function postAjaxForDeploy(params, checkedHostLength){
	...
	getJsonDataByPost(Globals.baseActionUrl.ROCKETMQ_TASK_ACTION_DEPLOY_INFOMATION_URL,params,"启停管理--rocketmq提交部署信息",
		function(result){
