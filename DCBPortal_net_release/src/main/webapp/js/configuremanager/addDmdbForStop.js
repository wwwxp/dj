...
function checkedchanged() {
	...
	if (hostArray != null && hostArray.length > 0) {
		var params = {
			...
			//DEPLOT_FILE_TYPE:mini.get("DEPLOY_FILE_TYPE").getValue(),
...
function loadStopData(e) {
	...
	var mergeCells2="HOST_IP,INST_NAME";
...
function selectChange(e) {
	var selData = e.selecteds;
	if (selData != null && selData.length > 0) {
		for (var i=0; i<selData.length; i++) {
			//如果选中的为主模式则关联选中所有子模式
			var deployFileType = selData[i]["DEPLOY_FILE_TYPE"];
			var instId = selData[i]["INST_ID"];
			if (deployFileType == "main_pattern") {
				var hostId = selData[i]["HOST_ID"];
				var fileName = selData[i]["FILE_NAME"];
				var gridData = mini.get("deploy_datagrid").getData();
				for (var j=0; j<gridData.length; j++) {
					if (gridData[j]["INST_ID"] != instId 
							&& gridData[j]["HOST_ID"] == hostId 
							&& (gridData[j]["DEPLOY_FILE_TYPE"] == "instance_pattern")) {
						mini.get("deploy_datagrid").setSelected(gridData[j]);
					}
				}
			}
		}
...
function postAjaxForDeploy(host_arry){
	...
	getJsonDataByPost(Globals.baseActionUrl.DMDB_TASK_ACTION_STOP_INFOMATION_URL,host_arry,"启停管理--停止dmdb",
		function(result){
