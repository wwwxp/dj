...
function addConfigRow(currentRow, hostId, hostIp, sshUser, isFull, fullObj, instId) {
	...
	var str = '<tr style="padding-top:10px;" time="' + timeSeq + '" id="tr_'+hostIp+'" data-HOST_IP="'+hostIp+'" data-HOST_ID="'+hostId+'" data-INST_ID="'+instId+'">'
			...
			+'<th><span class="fred">*</span>启动实例：</th>'
			+'<td>'
			+'	<input class="mini-combobox" id="instance_'+timeSeq+'" name="instance_'+timeSeq+'" popupWidth="100%" '
			+'		data="getInstanceList(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\')" textField="INSTANCE_NAME" valueField="INSTANCE_NAME" required="true" '
			...
			+' 		<div style="text-align:right;width:25%;cursor:pointer;float:left;margin:2px 5px;" title="添加" id="addRow_' + timeSeq + '" name="addRow_'+timeSeq+'" href="javascript:void(0)" onclick="addNextRow(this, \''+timeSeq+'\', \''+hostIp+'\', \''+hostId+'\', \''+sshUser+'\')"><img src="../../../images/deployHost/add_row.png" /></div>'
			+' 		<div style="text-align:center;width:25%;cursor:pointer;float:left;margin:2px 5px;" title="删除" id="removeRow_' + timeSeq + '" name="removeRow_'+timeSeq+'" href="javascript:void(0)" onclick="delCurrentRow(this, \''+timeSeq+'\', \''+hostIp+'\', \''+hostId+'\')"><img src="../../../images/deployHost/del_row.png" /></div>'
			+'		<div style="text-align:left;width:25%;cursor:pointer;float:left;margin:2px 5px;" title="最近一次操作日志" '
			+'			id="log_' + timeSeq + '" name="log_'+timeSeq+'" href="javascript:void(0)" '
			+'			onclick="showStartStepOperationLog(\''+param["CLUSTER_ID"]+'\', \''+param["CLUSTER_TYPE"]+'\', \''+hostId+'\', \'\', \'\', \''+timeSeq+'\')"><img src="../../..//images/comlog24.png" /></div>'
 ...
 * @param hostId 主机ID
 * @param instPath 实例路径
 * @param filename 实例配置文件名称
 * @param timeSeq 行标识
 */
function showStartStepOperationLog(clusterId, clusterType, hostId, instPath, filename, timeSeq) {
	var instName = mini.get("instance_" + timeSeq).getValue();
	if (instName == null || instName == '' || typeof(instName) == 'undefined') {
		showWarnMessageTips("请选择实例!");
		return;
	}
	showStartOperationLog(clusterId, clusterType, hostId, instName, instName, timeSeq);
}
...
function addConfigRow(currentRow, hostId, hostIp, sshUser, isFull, fullObj, instId) {
	...
	if (isFull) {
		mini.get("version_" + timeSeq).setValue(fullObj["VERSION"]);
		mini.get("instance_" + timeSeq).setValue(fullObj["INST_PATH"]);
...
function addOperator() {
	...
	$("#paramsTable>tbody:eq(0)>tr").each(function(index, item){
		...
		var instanceName = mini.get("instance_"+time).getValue();
	...
	for (var i=0; i<rowData.length; i++) {
		if (rowData[i]["VERSION"] == "" || rowData[i]["INSTANCE_NAME"] == "") {
            showWarnMessageTips("M2DB实例名称和版本均不能为空，请选择！");
			return;
		}
		
		var hostId = rowData[i]["HOST_ID"];
		var instanceName = rowData[i]["INSTANCE_NAME"];
		for (var j=0; j<rowData.length; j++) {
			var nextInstanceName = rowData[j]["INSTANCE_NAME"];
			var nextHostId = rowData[j]["HOST_ID"];
			if (i != j && (hostId == nextHostId && nextInstanceName == instanceName)) {
                showWarnMessageTips("同一台主机不能选择相同实例，主机IP【" + rowData[j]["HOST_IP"] + "】 请选择！");
				return;
			}
		}
	}
					...
					function success(result){
						if (result != null && result != '') {
							...
							for (var i=0; i<result.length; i++) {
								...
								//M2DB实例
								result[i]["INST_PATH"] = result[i]["CONFIG_PATH"];
 ...
 * @param hostId
 */
function loadInstanceList(clusterId, clusterType, hostId) {
	return getInstanceList(clusterId, clusterType);
}
...
function onSubmit(){
	...
	for (var i=0; i<params.length; i++) {
		if (params[i]["VERSION"] == "" || params[i]["INSTANCE_NAME"] == "") {
            showWarnMessageTips("M2DB实例名称和版本均不能为空，请选择！");
			return;
		}
		
		var hostId = params[i]["HOST_ID"];
		var instanceName = params[i]["INSTANCE_NAME"];
		for (var j=0; j<params.length; j++) {
			var nextInstanceName = params[j]["INSTANCE_NAME"];
			var nextHostId = params[j]["HOST_ID"];
			if (i != j && (hostId == nextHostId && nextInstanceName == instanceName)) {
                showWarnMessageTips("同一台主机不能选择相同实例，主机IP【" + params[j]["HOST_IP"] + "】 请选择！");
				return;
			}
		}
	}
...
function postAjaxForDeploy(params){
	...
	getJsonDataByPost(Globals.baseActionUrl.M2DB_TASK_ACTION_DEPLOY_INFOMATION_URL,params,"启停管理--启动M2DB",
			function(result){
