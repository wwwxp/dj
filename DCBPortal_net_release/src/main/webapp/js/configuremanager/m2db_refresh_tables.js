...
function queryM2dbHostDiv(checkHostId){
	...
	getJsonDataByPost(Globals.baseActionUrl.FRAME_QUERY_FOR_LIST_URL, params, "M2DB刷新数据-主机查询",
			function(result){
				var str = "";
...
function chooseToStart(){
	...
	if(host_arry.length>0){
		var info = {};
		var radio=$("input[name='searchRadio']:checked").val();
		info["radio"] = radio;
		if(radio == '1'){
			info["cmdParam"] = mini.get('tables').getValue();
		}else{
			var cmd = mini.get('cmdParam').getValue();
			if(cmd.length <1){
                showWarnMessageTips("请输入命令！");
				return;
			}
			info["cmdParam"] = cmd;
		}
		//将参数中的+进行转义处理
		if (info["cmdParam"] != null && info["cmdParam"] != undefined && info["cmdParam"] != '') {
			info["cmdParam"] = info["cmdParam"].replace(/\+/g, "%2B");
		}
        ...
        var isRestartProcess = mini.get("isRestartProcess");

        info["isRestartProcess"] = isRestartProcess.checked;
		...
		showConfirmMessageAlter("确定该操作？", function ok(){
			...
			var textValue = "启动开始时间: " + ((new Date(startTimes)).format("yyyy-MM-dd hh:mm:ss")) + "<br/>";
			textValue +="正在运行...<br/>";
			...
			getJsonDataByPost(Globals.baseActionUrl.HOSTSTART_ACTION_M2DB_REFRESH_URL,info,"启停管理-刷新表m2db",
				function(result){
