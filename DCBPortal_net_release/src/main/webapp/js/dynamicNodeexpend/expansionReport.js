...
function execManualJobKS() {
	...
	var params = {
		...
		THRESHOLD_TYPE:'expend',
		ID:JsVar["data"]["ID"],
		FORECAST_REPORT_ID:JsVar["data"]["FORECAST_REPORT_ID"]
	};
	showAddDialog("立刻扩展吗", 600, 420, Globals.baseJspUrl.NODE_STRATEGY_MANUAL_DYNAMIC_URL,
	        function destroy(data){
	            if (data == systemVar.SUCCESS) {
	            	 
	                //showMessageAlter("新增定时" + typeName + "策略成功！");
	            	showMessageTips("立即执行任务成功，任务下在执行中！");
	            }else{
	            	checkBtnTrue();
	            }
	    }, params);
...
function execJobKS(){
	...
	showAddDialog("定时扩展吗", 600, 420, Globals.baseJspUrl.NODE_STRATEGY_TIMING_DYNAMIC_URL,
	        function destroy(data){
	            if (data == systemVar.SUCCESS) {
	            	 
	                //showMessageAlter("新增定时" + typeName + "策略成功！");
	            	showMessageTips("新增定时扩展任务成功！");
	            }else{
	            	checkBtnTrue();
	            }
	    }, params);
