...
function loadZookeeperList() {
    ...
    if (chartsClusterData != null && chartsClusterData.length > 0) {
        ...
        intervalObj = setInterval(changeChartsClusterList, 5000);
    }

    var chartsClusterId = mini.get("CLUSTER_ID").getData();
    if (chartsClusterId != null && chartsClusterId.length > 0) {
        mini.get("CLUSTER_ID").select(0);
    }
}
...
function changeChartsClusterList(e) {
    var params = {
        //CLUSTER_ID:mini.get("CLUSTER_CHARTS_ID").getValue()
    ...
    getJsonDataByPost(Globals.baseActionUrl.TASK_OVERSTOCK_SERVICE_CHARTS_QUERY_URL, params, "任务积压查询-查询服务积压情况信息",
        function(result){
            ...
            if (result != null && result.length > 0) {
                ...
                showWarnMessageTips("没有数据!")
                //serviceCharts.dispose();
            }
 ...
 * @param e
 */
function changeServiceGroupList(e) {
    $("#groupServiceList").html("");
    var serviceData = mini.get("SERVICE_NAME").getData();
    var serviceGroup = mini.get("SERVICE_NAME").getValue();
    var params = {
        //CLUSTER_ID:mini.get("CLUSTER_ID").getValue(),
        CLUSTER_ID:mini.get("CLUSTER_ID").getSelected()["CLUSTER_ID"],      //zookeeper集群ID
        CLUSTER_CODE:mini.get("CLUSTER_ID").getSelected()["CLUSTER_CODE"],   //jstorm集群编码
    ...
    getJsonDataByPost(Globals.baseActionUrl.TASK_OVERSTOCK_SERVICE_GROUP_QUERY_URL, params, "任务运行积压查询-查询服务组对应的服务信息",
        function(result){
            if (result != null && result.length > 0) {
                $("#groupServiceList").html(result.join(","));
            }
        }, null, null, false);
}
...
function load(param){
    ...
    getJsonDataByPost(Globals.baseActionUrl.TASK_OVERSTOCK_SERVICE_DATA_QUERY_WITH_GROUP_URL, param, "任务运行积压查询-查询服务积压数据",
        function(result){
            // if (result != null && result.length > 0) {
                loadHostProgramList(result);
            // }
        }, null, null, false);
}
...
function changeHostList(e) {
    ...
    loadHostProgramList(tempData,"sort");
}
