...
function collectAxesInfo(result, ecModel, api) {
  ...
  each$10(api.getCoordinateSystems(), function (coordSys) {
    ...
    each$10(coordSys.getAxes(), curry$1(saveTooltipAxisInfo, false, null)); // If axis tooltip used, choose tooltip axis for each coordSys.
    ...
    && baseTooltipModel.get('show')) {
      ...
      if (triggerAxis || cross) {
        each$10(tooltipAxes.baseAxes, curry$1(saveTooltipAxisInfo, cross ? 'cross' : true, triggerAxis));
      }

      if (cross) {
        each$10(tooltipAxes.otherAxes, curry$1(saveTooltipAxisInfo, 'cross', false));
      }
...
var axisTrigger = function (payload, ecModel, api) {
  ...
  each$12(coordSysAxesInfo.coordSysMap, function (coordSys, coordSysKey) {
    // If a point given, it must be contained by the coordinate system.
    var coordSysContainsPoint = isIllegalPoint || coordSys.containPoint(point);
    each$12(coordSysAxesInfo.coordSysAxesInfo[coordSysKey], function (axisInfo, key) {
      var axis = axisInfo.axis;
  ...
  var linkTriggers = {};
  each$12(axesInfo, function (tarAxisInfo, tarKey) {
    var linkGroup = tarAxisInfo.linkGroup; // If axis has been triggered in the previous stage, it should not be triggered by link.

    if (linkGroup && !showValueMap[tarKey]) {
      each$12(linkGroup.axesInfo, function (srcAxisInfo, srcKey) {
  ...
  each$12(linkTriggers, function (val, tarKey) {
...
function buildPayloadsBySeries(value, axisInfo) {
  ...
  var minDist = Number.MAX_VALUE;
  var minDiff = -1;
  each$12(axisInfo.seriesModels, function (series, idx) {
    ...
    if (dist <= minDist) {
      ...
      each$12(dataIndices, function (dataIndex) {
        payloadBatch.push({
...
function updateModelActually(showValueMap, axesInfo, outputFinder) {
  ...
  each$12(axesInfo, function (axisInfo, key) {
...
function dispatchHighDownActually(axesInfo, dispatchAction, api) {
  ...
  each$12(axesInfo, function (axisInfo, key) {
    var option = axisInfo.axisPointerModel.option;
    option.status === 'show' && each$12(option.seriesDataIndices, function (batchItem) {
...
function initGlobalListeners(zr, api) {
  ...
  function useHandler(eventType, cb) {
    zr.on(eventType, function (e) {
      ...
      each$13(get$3(zr).records, function (record) {
...
function assembleFont(textStyleModel) {
  ...
  each$15(['decoration', 'align'], function (name) {
...
function assembleCssText(tooltipModel) {
  ...
  each$15(['width', 'color', 'radius'], function (name) {
...
extendComponentView({
  ...
  _showAxisTooltip: function (dataByCoordSys, e) {
    ...
    each$14(dataByCoordSys, function (itemCoordSys) {
      // var coordParamList = [];
      ...
      each$14(itemCoordSys.dataByAxis, function (item) {
  ...
  _updateContentNotChangedOnAxis: function (dataByCoordSys) {
    ...
    contentNotChanged && each$14(lastCoordSys, function (lastItemCoordSys, indexCoordSys) {
      var lastDataByAxis = lastItemCoordSys.dataByAxis || {};
      var thisItemCoordSys = dataByCoordSys[indexCoordSys] || {};
      var thisDataByAxis = thisItemCoordSys.dataByAxis || [];
      contentNotChanged &= lastDataByAxis.length === thisDataByAxis.length;
      contentNotChanged && each$14(lastDataByAxis, function (lastItem, indexAxis) {
        var thisItem = thisDataByAxis[indexAxis] || {};
        var lastIndices = lastItem.seriesDataIndices || [];
        var newIndices = thisItem.seriesDataIndices || [];
        contentNotChanged &= lastItem.value === thisItem.value && lastItem.axisType === thisItem.axisType && lastItem.axisId === thisItem.axisId && lastIndices.length === newIndices.length;
        contentNotChanged && each$14(lastIndices, function (lastIdxItem, j) {
