...
function onLoadComplete(action, data) {
	...
	if (data["THRESHOLD_TYPE"] == "expend") {
		mini.get("CONDITION_PARAM").setData(getSysDictData("condition_param_expend"));
	} else if (data["THRESHOLD_TYPE"] == "unexpend") {
		mini.get("CONDITION_PARAM").setData(getSysDictData("condition_param_unexpend"));
	}
...
function addThresholdConfig() {
    ...
    if (JsVar["DATA"]["THRESHOLD_TYPE"] == "expend") {
    	thresholdForm["OPERATOR_TYPE"] = "1";
    } else if (JsVar["DATA"]["THRESHOLD_TYPE"] == "unexpend") {
    	thresholdForm["OPERATOR_TYPE"] = "2";
    }
