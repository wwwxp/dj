...
function addTimingConfig() {
    ...
    //timingForm["STATE"] = '1';
    var timingObjs = [];
    var jobs = timingForm["CRON_EXP"].split(',');
    if(jobs.length>1){
    	for(var i = 0 ;i < jobs.length ;i++){
    		var tempObj = timingForm;
    		tempObj["JOB_ID"]=jobs[i];
    		timingObjs.push(tempObj);
    	}
    	
    }else{
    	timingForm["JOB_ID"] = timingForm["CRON_EXP"];
    	timingObjs.push(timingForm);
    }
    getJsonDataByPost(Globals.baseActionUrl.TIMING_CONFIG_ADD_URL, timingObjs, "节点伸缩策略配置-新增定时策略配置",
...
function updateTimingConfig() {
    ...
    getJsonDataByPost(Globals.baseActionUrl.TIMING_CONFIG_UPDATE_URL,timingForm,"节点伸缩策略配置-修改定时策略配置",
        function success(result){
            closeWindow(systemVar.SUCCESS);
        });
}
...
function loadStrategyConfig(id) {
	...
	getJsonDataByPost(Globals.baseActionUrl.FRAME_QUERY_FOR_OBJECT_URL, params, "节点伸缩策略配置-查询策略配置信息",
        function success(result){
            ...
            mini.get("CRON_EXP").setText(result["TASK_NAME"]);
            mini.get("CRON_EXP").setValue(result["JOB_ID"]);
            mini.get("BACKUP_HOSTS").setText(result["BACKUP_HOSTS"]);
            //mini.get("CRON_EXP").disable();
