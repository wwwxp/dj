...
export SENTINEL_HOME=$PROGPATH/env/$8/dca/sentinel
...
export MONITOR_DCA_HOME=$PROGPATH/env/$8/dca/monitorDCA
export SWITCH_DCA_HOME=$PROGPATH/env/$8/dca/switchDCA
...
################### DSF Framework  ##################
export DSF_HOME=$PROGPATH/env/$8/dsf
...
{
  echo -e "Usage: "
  echo -e "  ./$PROGNAME -s <rocketmq|fastdfs|dca|dmdb|monitor|dclog|zookeeper|m2db|jstorm|dsf> Failed."
  echo -e "  ./$PROGNAME -e <rocketmq|fastdfs|dca|dmdb|monitor|dclog|zookeeper|m2db|jstorm|dsf> Failed."
  echo -e "  ./$PROGNAME -d <rocketmq|fastdfs|dca|dmdb|monitor|dclog|zookeeper|m2db|jstorm|dsf> Failed."
}
...
StopNameServ()
{
	...
	instpath=$ROCKETMQ_HOME/conf/2m-2s-async/$ARG2
	cd $ROCKETMQ_HOME/bin;
	namesrvpid=`ps ux | grep -i 'org.apache.rocketmq.namesrv.NamesrvStartup' |grep java |grep $instpath | grep -v grep | awk '{print $2}'`
	echo -e "Namesrv PID -----> $namesrvpid"
	if [ "-$namesrvpid" != "-" ];
	then
		kill -9 $namesrvpid
...
StartDCAM()
{
	...
	then
		...
		echo -e "DCAM HOME ---> $DCAM_HOME"

		dcamPid=`ps ux |grep mserver|grep -v grep|grep $cfg/$ARG2/dcam.json|awk '{print $2}'`
		if [ "-$dcamPid" != "-" ];
		then
			echo "DCAM Server running, Server PID ---> $dcamPid"
			echo "Success."
		else
			cd $DCAM_HOME;
			nohup ./mserver $cfg/$ARG2/dcam.json $cfg/$ARG2/dcam.properties > $AUTO_SH_EXECPATH/nohup_sh.out 2>&1 &
			sleep 5
			cat $AUTO_SH_EXECPATH/nohup_sh.out

			dcamPid=`ps ux |grep mserver|grep -v grep|grep $cfg/$ARG2/dcam.json|awk '{print $2}'`
			echo -e "DCAM PID -----> $dcamPid"
			if [ "-$dcamPid" != "-" ];
			then
				echo " Success."
			else
...
StopDCAM()
{
	...
	then
		...
		cfg=$DCAM_HOME/config
		echo -e "Config Home ---> $cfg"
		echo -e "Instance Name ---> $ARG2"
		dcamPid=`ps ux |grep mserver|grep -v grep|grep $cfg/$ARG2/dcam.json|awk '{print $2}'`
		if [ "-$dcamPid" == "-" ];
		then
			echo "PID not exists, stop success."
			echo " Success."
		else
			kill -9 $dcamPid
			sleep 4
...
StartDCAS()
{
	...
	then
		...
		dcasPid=`ps ux | grep sserver | grep -v grep | grep $cfg/$ARG2/dcas.json | awk '{print $2}'`
		if [ "-$dcasPid" != "-" ];
		then
			echo "DCAS Server running, Server PID ---> $dcasPid"
			echo "Success."
		else
			echo -e "DCAS HOME ---> $DCAS_HOME"
			cd $DCAS_HOME;
			nohup ./sserver $cfg/$ARG2/dcas.json $cfg/$ARG2/dcas.properties > $AUTO_SH_EXECPATH/nohup_sh.out 2>&1 &
			sleep 5
			cat $AUTO_SH_EXECPATH/nohup_sh.out

			dcasPid=`ps ux | grep sserver | grep -v grep | grep $cfg/$ARG2/dcas.json | awk '{print $2}'`
			echo "DCAS PID -----> $dcasPid"
			if [ "-$dcasPid" != "-" ];
			then
				echo " Success."
			else
...
StopDCAS()
{
	...
	then
		...
		cfg=$DCAS_HOME/config
		echo -e "DCAS Config Home ---> $cfg"
		echo -e "DCAS Instance Name ---> $ARG2"
		dcasPid=`ps ux |grep sserver|grep -v grep|grep $cfg/$ARG2/dcas.json|awk '{print $2}'`
		if [ "-$dcasPid" == "-" ];
		then
			echo "PID not exists, stop success."
...
StartRedis()
{
	...
	then
		...
		cfg=$REDIS_HOME/$ARG2
		echo -e "Redis Config ---> $cfg"
        echo -e "Redis Host&Port&Passwd ---> $ARG4"
		if [ -e $cfg ];
		then
			rm -rf $cfg
		fi
		...
		echo -e "Redis Home ---> $REDIS_HOME"

		#检查redis是否有启动   192.168.161.89_8899#201911
		hostPasswd=${ARG4#*#}
		hostIpAndPort=${ARG4%%#*}
		hostPort=${hostIpAndPort#*_}
		hostIp=${hostIpAndPort%_*}
		echo -e "IP:$hostIp, PORT:$hostPort, PASS:$hostPasswd"

		redisMsg=`./redis-cli -p $hostPort -h $hostIp -a $hostPasswd ping`
		if [ "-$redisMsg" == "-PONG" ];
		then
		      echo "redis server is running..."
		      echo " Success."
		else
		   nohup ./redis-server $cfg --port $hostPort --dir ${cfg%/*} --requirepass $hostPasswd --masterauth $hostPasswd --bind $hostIp  > $AUTO_SH_EXECPATH/nohup_redis.out 2>&1 &
		   sleep 4
		   cat $AUTO_SH_EXECPATH/nohup_redis.out
		   redisMsg=`./redis-cli -p $hostPort -h $hostIp -a $hostPasswd ping`
           echo -e "check redis result ---> $redisMsg"
           if [[ "-$redisMsg" == "-PONG" ]] || [[ "-$redisMsg" == "-LOADING Redis is loading the dataset in memory" ]];
           then
           		echo " Success."
           else
           	    echo " Failed."
...
InitRedis()
{
	...
	then
		echo -e "Init Redis ......"
		cd $REDIS_HOME
        echo "Redis Home --> $REDIS_HOME"
        cd $REDIS_HOME/../bin
		./init $ARG4 $ARG2 > $AUTO_SH_EXECPATH/nohup_initRedis.out 2>&1 &
		sleep 4
        cat $AUTO_SH_EXECPATH/nohup_initRedis.out
	else
		echo "PARAM is null, Failed."
	fi
}
...
StopRedis()
{
	...
	then
		...
		echo -e "Redis HostIP&PORT&Passwd -----> $ARG2"
		cd $REDIS_HOME
        hostPasswd=${ARG2#*#}
        hostIpAndPort=${ARG2%%#*}
        hostPort=${hostIpAndPort#*_}
		...
		./redis-cli -p $hostPort -h $hostIp -a $hostPasswd shutdown
		echo -e "sh redis-cli -p $hostPort -h $hostIp -a $hostPasswd shutdown ...."
		sleep 4
		redispid=$hostPort
...
StartSentinel()
{
	if [ "-$ARG2" != "-" ];
	then
		echo -e "Start Sentinel Server ......"
		cfg=$SENTINEL_HOME/$ARG2
		echo -e "Sentinel Config ---> $cfg"
		echo -e "Sentinel Port ---> $ARG4"
		if [ -e $cfg ];
		then
			rm -rf $cfg
		fi
		...
		cd $SENTINEL_HOME
		ln -s $PROGPATH/conf/dca/sentinel/$ARG2 $ARG2
		echo -e "Redis Home ---> $REDIS_HOME"
		cd $REDIS_HOME

		#检查sentinel是否有启动
		redisMsg=`./redis-cli -p $ARG4 ping`
		if [ "-$redisMsg" == "-PONG" ];
		then
		      echo "Sentinel server is runeding..."
			  echo " Success."
		else
			nohup ./redis-server $cfg --sentinel --port $ARG4 > $AUTO_SH_EXECPATH/nohup_sentinel.out 2>&1 &
			sleep 4
			cat $AUTO_SH_EXECPATH/nohup_sentinel.out
...
StopSentinel()
{
	if [ "-$ARG2" != "-" ];
	then
		echo -e "Stop Sentinel Server ......"
		echo -e "Sentinel PORT -----> $ARG2"
		cd $REDIS_HOME
		./redis-cli -p $ARG2 shutdown
		sleep 4

		redisMsg=`./redis-cli -p $ARG2 ping`
		echo -e "check redis result ---> $redisMsg"
		if [ "-$redisMsg" != "-PONG" ];
		then
			echo " Sentinel is stop ... "
			echo " Success."
		else
			echo " Sentinel is stop ... "
			echo " Failed."
...
StartDaemon()
{
	...
	if [[ -e $DAEMON_DCA_HOME/config.js ]] || [[ -L $DAEMON_DCA_HOME/config.js ]];
	then
		rm -rf $DAEMON_DCA_HOME/config.js
		echo -e "init delete config.js"
	fi
	cd $DAEMON_DCA_HOME
...
StartDcaMonitor()
{
	echo -e "start dcaMonitor ......";
	if [[ -e $MONITOR_DCA_HOME/config.js ]] || [[ -L $MONITOR_DCA_HOME/config.js ]];
	then
		rm -rf $MONITOR_DCA_HOME/config.js
		echo -e "init delete config.js"
	fi
	cd $MONITOR_DCA_HOME
	ln -s $PROGPATH/conf/dca/monitorDCA/config.js config.js

	##create daemon log path
	logpath=`cat $PROGPATH/conf/dca/monitorDCA/config.js | grep customBaseDir | cut -d "'" -f 2 | cut -d "'" -f -1`
	echo -e "monitor logPath ---> $logpath"
	if [ "-$logpath" != "-" ];
	then
		mkdir -p $logpath
	fi

	daemonCluster=`cat $PROGPATH/conf/dca/monitorDCA/config.js | grep cluster | cut -d "'" -f 2 | cut -d "'" -f -1 | head -1`
   daemonPort=`cat $PROGPATH/conf/dca/monitorDCA/config.js | grep 'port :' | cut -d ":" -f 2 | cut -d "," -f -1 | head -1`
   daemonPort=`echo $daemonPort`
	echo -e "daemon daemonCluster ---> $daemonCluster"
   echo -e "daemon Port ---> $daemonPort"
   daemonName=$daemonCluster"_"$daemonPort
   echo -e "daemon deamonName ---> $daemonName"

	#daemon程序实例已经存在，先删掉
	daemonpid=`../pm2/bin/pm2 list | grep $daemonName | awk '{print $8}'`
    echo -e "deamonpid ---> $daemonpid"
	if [ "-$daemonpid" != "-" ];
	then
		../pm2/bin/pm2 delete $daemonName 2>&1
		sleep 1
	...
	../pm2/bin/pm2 -f -n $daemonName start app.js > $AUTO_SH_EXECPATH/nohup_sh.out 2>&1
	sleep 4
	cat $AUTO_SH_EXECPATH/nohup_sh.out
	dcadaemonid=`ps ux | grep node | grep $MONITOR_DCA_HOME/app.js | grep -v debug-port |awk '{print $2}'`
	echo -e "DCA DAEMON PID -----> $dcadaemonid"
	if [ "-$dcadaemonid" != "-" ];
	then
		echo " Success."
...
StopDcaMonitor()
{
   ...
   echo -e "daemon deamonName ---> $daemonName"
	...
	sleep 4
	cat $AUTO_SH_EXECPATH/nohup_sh.out
	dcadaemonid=`ps ux | grep node | grep $MONITOR_DCA_HOME/app.js |grep -v grep|awk '{print $2}'`
	if [ "-$dcadaemonid" == "-" ];
	then
		echo " Success."
	else
...
StartDcaSwitch()
{
	echo -e "start dcaSwitch ......";
	if [[ -e $SWITCH_DCA_HOME/config.js ]] || [[ -L $SWITCH_DCA_HOME/config.js ]];
	then
		rm -rf $SWITCH_DCA_HOME/config.js
		echo -e "init delete config.js"
	fi
	cd $SWITCH_DCA_HOME
	ln -s $PROGPATH/conf/dca/switchDCA/config.js config.js

	##create daemon log path
	logpath=`cat $PROGPATH/conf/dca/switchDCA/config.js | grep customBaseDir | cut -d "'" -f 2 | cut -d "'" -f -1 |head -1`
	echo -e "monitor logPath ---> $logpath"
	if [ "-$logpath" != "-" ];
	then
		mkdir -p $logpath
	...
	daemonName=`cat $PROGPATH/conf/dca/switchDCA/config.js | grep cluster | cut -d "'" -f 2 | cut -d "'" -f -1|head -1`
	echo -e "daemon daemonName ---> $daemonName"
	...
	sleep 4
	cat $AUTO_SH_EXECPATH/nohup_sh.out
	dcadaemonid=`ps ux | grep node | grep $SWITCH_DCA_HOME/app.js | grep -v debug-port |awk '{print $2}'`
	echo -e "DCA SWITCH PID -----> $dcadaemonid"
	if [ "-$dcadaemonid" != "-" ];
	then
		echo " Success."
	else
...
StopDcaSwitch()
{
	echo -e "Stop dcaSwitch ......"
	cd $SWITCH_DCA_HOME;
	daemonName=`cat $PROGPATH/conf/dca/switchDCA/config.js | grep cluster | cut -d "'" -f 2 | cut -d "'" -f -1`
	echo -e "DCA daemon AppName ---> $daemonName"
	../pm2/bin/pm2 delete $daemonName > $AUTO_SH_EXECPATH/nohup_sh.out 2>&1
	sleep 4
	cat $AUTO_SH_EXECPATH/nohup_sh.out
	dcadaemonid=`ps ux | grep node | grep $SWITCH_DCA_HOME/app.js |grep -v grep|awk '{print $2}'`
	if [ "-$dcadaemonid" == "-" ];
	then
		echo " Success."
	else
...
#================ DSF Framework Start And Stop ======================================#
StartDSF()
{
	if [ "-$ARG3" != "-" ];
	then
		echo -e "start dsf framework ......";
		if [[ -e $DSF_HOME/conf/application.properties ]] || [[ -L $DSF_HOME/conf/application.properties ]];
		then
			rm -rf $DSF_HOME/conf/application.properties
			echo -e "init delete application.properties"
		fi
		if [[ -e $DSF_HOME/conf/config.properties ]] || [[ -L $DSF_HOME/conf/config.properties ]];
		then
			rm -rf $DSF_HOME/conf/config.properties
			echo -e "init delete config.properties"
		fi
		if [[ -e $DSF_HOME/conf/logback.xml ]] || [[ -L $DSF_HOME/conf/logback.xml ]];
		then
			rm -rf $DSF_HOME/conf/logback.xml
			echo -e "init delete logback.xml"
		fi
		ln -s $PROGPATH/conf/dsf/application.properties $DSF_HOME/conf/application.properties
		ln -s $PROGPATH/conf/dsf/config.properties $DSF_HOME/conf/config.properties
		ln -s $PROGPATH/conf/dsf/logback.xml $DSF_HOME/conf/logback.xml

		cd $DSF_HOME/bin
		nohup sh run.sh > $AUTO_SH_EXECPATH/nohup_sh.out 2>&1 &
        sleep 5
		#cat $AUTO_SH_EXECPATH/nohup_sh.out
        agentPid=`ps ux | grep AgentMain|grep -v grep|awk '{print $2}'`
        echo -e "AgentMain Process ID ---> $agentPid"
        if [ "-$agentPid" != "-" ];
        then
            echo " Success."
        else
...
StopDSF()
{
	...
	then
		...
		if [ "-$agentPid" == "-" ];
        then
            echo " AgentWorker not running ......"
            echo " Success."
        else
            kill -9 $agentPid
            sleep 2
            agentNewPid=`ps ux |grep AgentMain|grep -v grep|awk '{print $2}'`
            if [ "-$agentNewPid" == "-" ];
            then
                echo " Success."
            else
...
StartProgrammerMonitor()
{
	...
	then
		...
		then
			...
			if [ -e $MONITOR_COMPONENT_HOME/zookeeper/zookeeper.properties ];
            then
                rm -rf $MONITOR_COMPONENT_HOME/zookeeper/zookeeper.properties
            fi
			...
			ln -s $PROGPATH/conf/monitor/zookeeper.properties zookeeper/zookeeper.properties
...
StartDMDB()
{
	if [ "-$ARG2" != "-" ]
	then
		#去除最后面的/及空格，如default/  目录，防止删除软连接目录时将源文件删除
		ARG2=`echo $ARG2 | sed 's/\/ *$//'`
	...
	case $ARG in
		instance_pattern|route_pattern|sync_pattern|mgr_pattern|watcher_pattern|movesync_pattern)
			echo -e "Start dmdb $ARG ......"
			cfgDir=$ARG/$inst_dir_name
			;;
		*)
			echo "$ARG is not a supported param Failed.";
			return 1
	...
	cd $DMDB_HOME
	instpath=$DMDB_HOME/cfg/$cfgDir
	echo "Instance Path ------> $instpath"
	if [ -L $instpath ]
	then
		rm -rf $instpath
	fi

	ln -s $PROGPATH/conf/dmdb/$ARG2 $instpath

	cd $DMDB_HOME
	chmod +x *.sh
	./boot.sh $ARG $instpath >$AUTO_SH_EXECPATH/nohup_sh.out 2>&1
	sleep 2
	cat $AUTO_SH_EXECPATH/nohup_sh.out

	#检查是否已经正常启动#
	./check.sh $ARG $instpath >$AUTO_SH_EXECPATH/nohup_check_sh.out 2>&1
	if [ $? = 1 ]
	then
		echo " Success."
	else
...
StopDMDB()
{
	...
	echo "Instance Path -----> $instpath"
	chmod +x *.sh
	./stop.sh $ARG $instpath > $AUTO_SH_EXECPATH/nohup_sh.out 2>&1
	sleep 3
	cat $AUTO_SH_EXECPATH/nohup_sh.out
...
StartNimbus()
{
	...
	if [[ -L $JSTORM_HOME/conf/storm.yaml ]] || [[ -e $JSTORM_HOME/conf/storm.yaml ]];
	then
		rm -rf $JSTORM_HOME/conf/storm.yaml
		echo -e "init delete storm.yaml"
	fi
	if [[ -L $JSTORM_HOME/conf/jstorm.logback.xml ]] || [[ -e $JSTORM_HOME/conf/jstorm.logback.xml ]];
	then
		rm -rf $JSTORM_HOME/conf/jstorm.logback.xml
		echo -e "init delete jstorm.logback.xml"
	fi
...
StopSupervisor()
{
	...
	workerarr=(`jps |grep Worker|grep -v grep|awk '{print $1}'`)
	supervisorparr=(`jps |grep Supervisor|grep -v grep|awk '{print $1}'`)
	pidarr=(`ps -ef|grep $who|grep cstorm|grep -v grep|awk '{print $2}'`)
