...
InitRedis()
{
	...
	then
        ...
        echo -e "ARG4 ---> $ARG4"
        ##clusterCode=clusterCode%netcard
        echo -e "ARG2 ---> $ARG2"
        clusterCode=${ARG2%\%*}
        netcard=${ARG2##*%}
        echo -e "clusterCode --->$clusterCode"
        echo -e "netCard ---> $netcard"
        cd $REDIS_HOME/../bin
		./init $ARG4 $clusterCode $netcard > $AUTO_SH_EXECPATH/nohup_initRedis.out 2>&1 &
		sleep 4
...
StartSentinel()
{
	...
	then
		...
		echo -e "Sentinel Host&Port ---> $ARG4"
		if [ -e $cfg ];
		then
			rm -rf $cfg
		fi
		...
		#检查sentinel是否有启动
		redisMsg=`./redis-cli -p $port -h $host ping`
		if [ "-$redisMsg" == "-PONG" ];
		then
		      echo "Sentinel server is running..."
			  echo " Success."
		else
			nohup ./redis-server $cfg --sentinel --bind $host --port $port > $AUTO_SH_EXECPATH/nohup_sentinel.out 2>&1 &
			sleep 4
...
StopSentinel()
{
	...
	then
		...
		echo -e "Sentinel HOST&PORT -----> $ARG2"
		...
		cd $REDIS_HOME
		./redis-cli -p $port -h $host shutdown
		sleep 4
...
StartDcaMonitor()
{
    ...
    then
        ...
        cd $cfg
        ln -s $PROGPATH/conf/dca/monitorDCA/$ARG2/* $ARG2

        dcamonitorPid=`ps ux | grep monitorDCA | grep -v grep | grep $cfg/$ARG2/monitor.json | awk '{print $2}'`
        if [ "-$dcamonitorPid" != "-" ];
        then
            echo "DCA Monitor running, Server PID ---> $dcamonitorPid"
            echo "Success."
        else
            echo -e "DCA MONITOR HOME ---> $MONITOR_DCA_HOME"
            cd $MONITOR_DCA_HOME;
            nohup ./monitorDCA $cfg/$ARG2/monitor.json $cfg/$ARG2/monitor.properties > $AUTO_SH_EXECPATH/nohup_sh.out 2>&1 &
            sleep 5
            cat $AUTO_SH_EXECPATH/nohup_sh.out

            dcamonitorPid=`ps ux | grep monitorDCA | grep -v grep | grep $cfg/$ARG2/monitor.json | awk '{print $2}'`
            echo "DCA MONITOR PID -----> $dcamonitorPid"
            if [ "-$dcamonitorPid" != "-" ];
            then
                echo " Success."
            else
...
StopDcaMonitor()
{
    ...
    then
        ...
        cfg=$MONITOR_DCA_HOME/config
        echo -e "DCA Monitor Config Home ---> $cfg"
        echo -e "DCA Monitor Instance Name ---> $ARG2"
        dcamonitorPid=`ps ux | grep monitorDCA |grep -v grep|grep $cfg/$ARG2/monitor.json|awk '{print $2}'`
        if [ "-$dcamonitorPid" == "-" ];
        then
            echo "PID not exists, stop success."
...
StartDcaSwitch()
{
    ...
    then
        ...
        cd $cfg
        ln -s $PROGPATH/conf/dca/switchDCA/$ARG2/* $ARG2

        dcaswitchPid=`ps ux | grep switchDCA | grep -v grep | grep $cfg/$ARG2/switch.json | awk '{print $2}'`
        if [ "-$dcaswitchPid" != "-" ];
        then
            echo "DCA Switch running, Server PID ---> $dcaswitchPid"
            echo "Success."
        else
            echo -e "DCA SWITCH HOME ---> $SWITCH_DCA_HOME"
            cd $SWITCH_DCA_HOME;
            nohup ./switchDCA $cfg/$ARG2/switch.json $cfg/$ARG2/switch.properties > $AUTO_SH_EXECPATH/nohup_sh.out 2>&1 &
            sleep 5
            cat $AUTO_SH_EXECPATH/nohup_sh.out

            dcaswitchPid=`ps ux | grep switchDCA | grep -v grep | grep $cfg/$ARG2/switch.json | awk '{print $2}'`
            echo "DCA SWITCH PID -----> $dcaswitchPid"
            if [ "-$dcaswitchPid" != "-" ];
            then
                echo " Success."
            else
...
StopDcaSwitch()
{
    ...
    then
        ...
        cfg=$SWITCH_DCA_HOME/config
        echo -e "DCA Switch Config Home ---> $cfg"
        echo -e "DCA Switch Instance Name ---> $ARG2"
        dcaswitchPid=`ps ux | grep switchDCA |grep -v grep|grep $cfg/$ARG2/switch.json|awk '{print $2}'`
        if [ "-$dcaswitchPid" == "-" ];
        then
            echo "PID not exists, stop success."
...
CheckSwitchDCA()
{
	...
	then
		cd $SWITCH_DCA_HOME;
		realPath=$PROGPATH/conf/dca/switchDCA/$ARG2
		daemonName=`cat $realPath | grep cluster | cut -d "'" -f 2 | cut -d "'" -f -1|head -1`
		if [ "-$daemonName" != "-" ];
		then
		    switchpid=`../pm2/bin/pm2 list | grep " $daemonName " | awk '{print $8}'`
		    echo "$switchpid"
		else
		    echo ""
		fi
...
CheckMonitorDCA()
{
	...
	then
		cd $MONITOR_DCA_HOME;
		realPath=$PROGPATH/conf/dca/monitorDCA/$ARG2
		daemonCluster=`cat $realPath | grep cluster | cut -d "'" -f 2 | cut -d "'" -f -1 | head -1`
        daemonPort=`cat $realPath | grep 'port :' | cut -d ":" -f 2 | cut -d "," -f -1 | head -1`
