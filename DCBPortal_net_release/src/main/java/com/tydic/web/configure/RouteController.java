...
public class RouteController extends BaseController {
    ...
    public String updateRunProgram(HttpServletRequest request) {
        ...
        try {
        	...
        	resultMap = routeService.updateRunAndStopHost(param, getDbKey(request));
        } catch (Exception e) {
            log.error("Route程序启动异常， 异常原因: ", e);
            resultMap.put("info", "启动失败");
            resultMap.put("flag", "error");
            resultMap.put("reason", e.getMessage());
        }
        log.debug("Route程序启动结束...");
    ...
    public String updateStopProgram(HttpServletRequest request) {
        ...
        try {
            ...
            log.error("Route程序停止异常， 异常原因: ", e);
            resultMap.put("info", "停止失败");
            resultMap.put("flag", "error");
            resultMap.put("reason", e.getMessage());
        }
        log.debug("Route程序停止结束...");
        return JSON.Encode(resultMap);
    }
	...
	public String updatecheck(HttpServletRequest request) {
		log.debug("Route程序状态检查开始...");
		Map<String, Object> result = new HashMap<String, Object>();
		try {
			Map<String, Object> param = this.getParamsMapByObject(request);
			result = routeService.updateCheckHostState(param, getDbKey(request));
		} catch (Exception e) {
			log.error("Route程序状态检查失败， 失败原因: ", e);
			return JSON.Encode(ResponseObj.getResultMap(ResponseObj.ERROR, e.getMessage()));
		}
		log.debug("Route程序状态检查结束...");
	...
	public String updateSwitch(HttpServletRequest request) {
		log.debug("Route程序主备切换开始...");
		Map<String, Object> result = new HashMap<String, Object>();
		try {
			result = routeService.updateSwitch(getDbKey(request));
		} catch (Exception e) {
			log.error("Route程序主备切换异常， 异常原因: ", e);
			return JSON.Encode(ResponseObj.getResultMap(ResponseObj.ERROR, e.getMessage()));
		}
		log.debug("Route程序主备切换结束...");
		return JSON.Encode(result);
	}
}
