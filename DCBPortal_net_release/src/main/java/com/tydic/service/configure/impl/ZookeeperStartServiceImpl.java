 ...
 * 
  ...
  * @Package:      [com.tydic.service.configure.impl]    
  * @ClassName:    [ZookeeperStartServiceImpl]     
  * @Description:  [Zookeeper组件启停实现类]     
  * @Author:       [Yuanh]     
  * @CreateDate:   [2017-6-13 下午5:16:07]     
  * @UpdateUser:   [Yuanh]     
  * @UpdateDate:   [2017-6-13 下午5:16:07]     
  * @UpdateRemark: [说明本次修改内容]    
  * @Version:      [v1.0]   
  *
 */
@Service
public class ZookeeperStartServiceImpl implements ZookeeperStartService {
	/**
	 * Zookeeper启停日志
	 */
	private static Logger logger = Logger.getLogger(ZookeeperStartServiceImpl.class);
	...
	*/
	 ...
	 */
	@Override
	public Map<String, Object> startZookeeper(List<Map<String, String>> param, final String dbKey) throws Exception {
		logger.debug("启动Zookeeper,参数 ---> " + param + ", dbKey ---> " + dbKey);
		//主机处理结果保存对象
		...
		if(!BlankUtil.isBlank(param)){
			...
			try {
					...
					pool.execute(new Runnable() {
						public void run() {
							...
							try {
								...
								//启停配置文件
								final String filePath = "zoo.cfg";
								//获取启停命令
								// chmod a+x auto.sh;./auto.sh -s ${CLUSTER_TYPE} -2 ${tracker} -3 ${配置文件} -4 ${version}
								final String autoFilePath = StringTool.object2String(hostMap.get("autoFile"));
								final String deployFile = MessageFormat.format(autoFilePath, Constant.ZOOKEEPER, Constant.ZOOKEEPER, appRootPath, version, StringTool.object2String(System.nanoTime()));
								logger.debug("Zookeeper 启停脚本命令 ---> " + deployFile);

								String hostId = hostMap.get("HOST_ID");
								//配置文件真实路径
								String cfgFilePath = appRootPath + Constant.Tools + Constant.CONF + Constant.ZOOKEEPER_DIR + filePath;

								//线程日志文件目录
								String logPath = instConfigService.getLogPath(clusterType);
								logger.info("start zookeeper logPath--->" + logPath);

								//线程日志文件
								Map<String, Object> logNameMap = instConfigService.getLogName(clusterId, clusterType, hostId, version, clusterType, cfgFilePath);
								...
								logger.info("start zookeeper logShaName--->" + logShaName + ", logStrName--->" + logStrName);

								String logFinalPath = StringUtils.removeEnd(logPath, "/") + "/" + logShaName;
								threadLogger = LoggerUtils.getThreadLogger(logPath, logShaName, "ZookeeperStartServiceImpl-Start-" + Thread.currentThread().getName());
								threadLogger.info("start zookeeper, logName: " + logFinalPath + ", startTime:" + DateUtil.getCurrent(DateUtil.allPattern) + ", operation userID:" + empeeId);
								threadLogger.info("clusterId: " + clusterId + ", hostId: " + hostId + ", version: " + version + ", deployType: " + clusterType + ", startAutoFilePath: " + cfgFilePath);
								threadLogger.info("logName source --->" + logStrName);

								//获取启停主机信息
								Map<String, Object> queryMap = new HashMap<String, Object>();
								queryMap.put("HOST_ID", hostId);
								threadLogger.info("query host, params: " + queryMap.toString());
								tempMap = coreService.queryForObject2New("host.queryHostList", queryMap, dbKey);
								threadLogger.info("query host, result: " + tempMap);

								//远程SSH主机信息
								String sshIp = StringTool.object2String(tempMap.get("HOST_IP"));
								String sshUser = StringTool.object2String(tempMap.get("SSH_USER"));
								String sshPass = DesTool.dec(StringTool.object2String(tempMap.get("SSH_PASSWD")));
								...
								try {
									threadLogger.debug("create pid file, ssh host, sshIp: " + sshIp + ", sshUser: " + sshUser + ", sshPasswd: " + sshPass + ", ftpType: sftp");
									ftpClient = FTPUtils.getFtpInstance(sshIp, sshUser, sshPass, "sftp");
									ftpClient.login();
									threadLogger.debug("create pid file, sftp connect&login success.");
									InputStream inputStream = ftpClient.get(cfgFilePath);
									Properties prop = PropertiesUtils.getPropertiesFileByInputStream(inputStream);
									ftpClient.completePendingCommand();
									String dataDir = prop.getProperty("dataDir");
									threadLogger.debug("create pid file, dataDir: " + dataDir);

									logger.debug("dataDir------->" + dataDir);
									if (StringUtils.isNotBlank(dataDir)) {
										boolean isExist = ftpClient.isExistPath(FileTool.exactPath(dataDir) + "myid");
										threadLogger.debug("create pid file, myid exists: " + isExist);

										//如果不存在myid文件，则创建myid文件
										if (!isExist) {
											String serverId = sshIp.substring(sshIp.lastIndexOf(".") + 1, sshIp.length());
											threadLogger.debug("create pid file, serverId: " + serverId);
											String myidCommand = "mkdir -p " + dataDir + ";echo " + serverId + " > " + dataDir + "/myid";
											threadLogger.debug("create pid file, exeCmd: " + myidCommand);
											threadLogger.debug("create pid file, sshIp: " + sshIp + ", sshUser:" + sshUser + ", sshPasswd: " + sshPass);
											ShellUtils cmdUtil = new ShellUtils(sshIp, sshUser, sshPass);
											String exeRet = cmdUtil.execMsg(myidCommand);
											threadLogger.debug("create pid file, result: " + exeRet);
											logger.debug("create [myid] folder【" + dataDir + "】-------------------- Success");
										}
									}
								} catch (Exception e) {
									threadLogger.error("create pid file failed, Cause by:", e);
									resultMap.put(Constant.RST_CODE, Constant.RST_CODE_FAILED);
									resultMap.put(Constant.RST_STR, e.getMessage());
									...
									if (ftpClient != null) {
										ftpClient.close();
										threadLogger.debug("create pid file, close sftp connect success");
									}
								}
								...
								if (isError) {
									...
									resultMap.put(Constant.RST_CODE, Constant.RST_CODE_FAILED);
									resultMap.put(Constant.RST_STR, "zookeeper startup fail, create pid file failed!!" );
									resultList.add(resultMap);

									threadLogger.warn("create pid file failed, zookeeper start failed, return!!");
									Long endProTimes = System.currentTimeMillis();
									Long totalProTimes = (endProTimes - startProTimes)/1000;
									threadLogger.info("start zookeeper final, endTime: " + (DateUtil.getCurrent(DateUtil.allPattern)) + ", totalTime: <label style='color:red'>" + totalProTimes + "</label>S");
									return;
								}
								//执行命令
								String command = "cd " + appRootPath + Constant.Tools + ";" + deployFile;

								ShellUtils cmdUtil = new ShellUtils(sshIp, sshUser, sshPass);
								String result = "";
								try {
									logger.debug(clusterType + "执行命令: " + command);
									threadLogger.info("sshIp:" + sshIp + ", sshUser: " + sshUser + ", sshPasswd: " + sshPass);
									threadLogger.info("exeCmd: <label style='color:green;'>" + command + "</label>");
									result = cmdUtil.execMsg(command);
									logger.debug(clusterType + "执行结果: " + result);
									threadLogger.info("result: " + result);
								} catch (Exception e) {
									threadLogger.error("start zookeeper failed, cause by: ", e);
									logger.error(clusterType + "启动异常，异常原因: ", e);
									result += "\n";
									result += " Failed.";
									result += "\n";
									result += e.getMessage();
								...
								if (result.indexOf(Constant.SUCCESS) < 0) {
									threadLogger.error("start zookeeper failed, cause by: " + result);
									resultMap.put(Constant.RST_CODE, Constant.RST_CODE_FAILED);
								} else {
									threadLogger.info("start zookeeper success!!");
									//该参数是用来添加到启动记录表
									resultMap.put("DEPLOY_FILE_TYPE", clusterType);
									//配置文件真实路径
									String completFilePath = appRootPath + Constant.Tools + Constant.CONF + Constant.ZOOKEEPER_DIR + filePath;
									...
									//配置文件软连接路径
									String softLinkPath = appRootPath + Constant.Tools + Constant.ENV + FileTool.exactPath(version) + Constant.ZOOKEEPER_DIR + Constant.CONF + instPath;
								...
								logger.error("Zookeeper启动失败， 失败原因: ", e);
								resultMap.put("HOST_IP", (tempMap == null ? "" : StringTool.object2String(tempMap.get("HOST_IP"))));
								resultMap.put(Constant.RST_CODE, Constant.RST_CODE_FAILED);
								resultMap.put(Constant.RST_STR, "zookeeper startup fail, cause: " + e.getMessage());
								resultList.add(resultMap);
				...
				while(resultList.size() < param.size()){
					logger.debug("本次总启动Zookeeper进程数:" + param.size() + ", 已经启动完成Zookeeper进程数:" + resultList.size());
				...
				logger.info("Zookeeper进程启动完成，本次启动Zookeeper进程数: " + param.size() + ", 总耗时: [ " + totalTimes + " ]S");
				...
				for(int i = 0 ; i < resultList.size();i++){
					Map<String, Object> singleMap = new HashMap<String, Object>();
					  ...
					  if(Constant.RST_CODE_SUCCESS.equalsIgnoreCase(resultMap.get(Constant.RST_CODE))){
						  ...
						  fileMap.put("DEPLOY_TYPE", clusterType);
				...
				logger.error("启动Zookeeper失败, 失败原因: ", e);
			}
		}
		return rstMap;
 	}
	 ...
	 */
	@Override
	public Map<String, Object> stopZookeeper(List<Map<String, String>> param, String dbKey) throws Exception {
		logger.debug("停止Zookeeper， 参数 ---> " + param + ", dbKey ---> " + dbKey);
		//主机处理结果保存对象
		List<Map<String, Object>> resultMsgList = new ArrayList<Map<String, Object>>();
		//返回对象
		Map<String, Object> rstMap = new HashMap<String, Object>();
		...
		if(!BlankUtil.isBlank(param)){
			...
			try {
					...
					pool.execute(new Runnable() {
						public void run() {
							...
							try {
								...
								//格式化后的执行命令
								final String autoFile = MessageFormat.format(executeCmd, Constant.ZOOKEEPER, deployFileType, instName, version);
								//根据主机ID查询主机信息
								...
								//日志文件路径
								String logPath = instConfigService.getLogPath(clusterType);
								logger.info("stop zookeeper logPath--->" + logPath);
								...
								logger.info("stop zookeeper logShaName--->" + logShaName + ", logStrName--->" + logStrName);

								String logFinalPath = StringUtils.removeEnd(logPath, "/") + "/" + logShaName;
								threadLogger = LoggerUtils.getThreadLogger(logPath, logShaName, "ZookeeperStartServiceImpl-Stop-" + Thread.currentThread().getName());
								threadLogger.info("stop zookeeper, logName: " + logFinalPath + ", startTime:" + DateUtil.getCurrent(DateUtil.allPattern) + ", operation userID:" + empeeId);
								threadLogger.info("logName source: " + logStrName);
								...
								String result = "";
								try {
									logger.debug("Zookeeper停止命令,执行命令: " + command);
									threadLogger.info("exeCmd: <label style='color:green;'>" + command + "</label>");
									result = cmdUtil.execMsg(command);
									logger.debug("Zookeeper停止命令,执行结果: " + result);
									threadLogger.info("result: " + result);
								} catch (Exception e) {
									threadLogger.error("stop zookeeper failed, cause by:", e);
									logger.error(clusterType + "停止异常，异常原因: ", e);
									result += "\n";
									result += " Failed.";
									result += "\n";
									result += e.getMessage();
								...
								if (result.indexOf(Constant.SUCCESS) < 0) {
									resultMap.put(Constant.RST_CODE, Constant.RST_CODE_FAILED);
									threadLogger.error("stop zookeeper failed, cause by: " + result);
								} else {
									resultMap.put(Constant.RST_CODE, Constant.RST_CODE_SUCCESS);
									threadLogger.info("stop zookeeper success!!");
								}
								//将脚本输出信息中的空行和错误标志给替换
								result = result.replaceAll(Constant.FLAG_RECOVERT_ERROR, "").replaceAll("((\r\n)|\n)[\\s\t ]*(\\1)+", "$1").replaceAll("^((\r\n)|\n)", "");
								resultMap.put(Constant.RST_STR, result);
								...
								long endProTimes = System.currentTimeMillis();
								long totalProTimes = (endProTimes - startProTimes)/1000;
								threadLogger.debug("stop zookeeper final, endTime: " + (DateUtil.getCurrent(DateUtil.allPattern)) + ", totalTime: <label style='color:red'>" + totalProTimes + "</label>S");
							} catch (Exception e) {
								logger.error("Zookeeper程序停止失败， 失败原因: ", e);
								resultMap.put(Constant.RST_CODE, Constant.RST_CODE_FAILED);
								resultMap.put(Constant.RST_STR, "zookeeper failed to stop, cause: " + e.getMessage());
				...
				while(resultList.size() < param.size()){
					logger.debug("本次总停止Zookeeper进程数:" + param.size() + ", 已经停止完成Zookeeper进程数:" + resultList.size());
					SingletonThreadPool.getExecutorService();
					Thread.sleep(2000);
				}
				...
				Long endTimes = System.currentTimeMillis();
				Long totalTimes = (endTimes - startTimes)/1000;
				logger.info("Zookeeper进程停止完成，本次停止Zookeeper进程数: " + param.size() + ", 总耗时: [ " + totalTimes + " ]S");
