...
import com.tydic.service.configure.RouteService;
 ...
 * 
  ...
  * @Description:  [Route业务启停实现类]     
  * @Author:       [Yuanh]     
  * @CreateDate:   [2017-6-13 下午5:14:43]     
  * @UpdateUser:   [Yuanh]     
  * @UpdateDate:   [2017-6-13 下午5:14:43]     
...
public class RouteServiceImpl implements RouteService {
	...
	public Map<String, Object> updateRunAndStopHost(Map<String, Object> param, String dbKey) throws Exception {
		log.debug("Route程序启停， 业务参数: " + param.toString() + ", dbKey: " + dbKey);
		//返回对象
		...
		String runAndStop = "启动";
		 ...
		 try {
			...
			for (Map<String, Object> paramMap : paramList) {
				// 查询出数据库得到脚本名称
				HashMap<String, Object> resultMap = coreService.queryForObject2New("programDefine.queryProgramListForRouter", paramMap, dbKey);
				String shName = StringTool.object2String(resultMap.get("SCRIPT_SH_NAME"));
				...
				log.debug("Route程序启停命令: " + execCmd);
				
				// 执行命令返回结果
				resultStr = cmdUtil.execMsg(execCmd);
				log.debug("Route程序启停结果: " + resultStr);
				...
				updateMap.put("PROGRAM_NAME", StringTool.object2String(resultMap.get("PROGRAM_NAME")));
				if(BusinessConstant.PARAMS_START_FLAG.equals(param.get("flag"))) {
					String programId = StringTool.object2String(paramMap.get("PROGRAM_ID"));
					if (BlankUtil.isBlank(programId) || BusinessConstant.PARAMS_UNDEFINED.equals(programId)) {
						...
						paramMap.put("PROGRAM_NAME", StringTool.object2String(resultMap.get("PROGRAM_NAME")));
	...
	public Map<String, Object> updateCheckHostState(Map<String, Object> params, String dbKey) throws Exception {
		log.debug("检查Route程序运行状态, 参数: " + params.toString() + ", dbKey: " + dbKey);
		//业务集群ID
		...
		try {
			...
			String hostUser = StringTool.object2String(hostMap.get("SSH_USER"));
			String hostPwd = DesTool.dec(StringTool.object2String(hostMap.get("SSH_PASSWD")));
			SSHRemoteCmdUtil cmdUtil = new SSHRemoteCmdUtil(hostIp, hostUser, hostPwd, null);
			...
			execCmd = execCmd.replace("$P", busCfgPath);
			log.debug("Route程序状态检查, 执行命令: " + execCmd);
			// 执行命令返回结果
			String resultStr = cmdUtil.execMsg(execCmd);
			log.debug("Route程序状态检查执行命令结果; " + resultStr);
	
			if(resultStr.toLowerCase().indexOf(ResponseObj.ERROR) >=0 
					 || resultStr.toLowerCase().indexOf(ResponseObj.FAILED) >=0){
				log.info("Route程序状态检查失败：" + resultStr);
			...
			if(resultStr.toLowerCase().contains(BusinessConstant.PARAMS_START_FLAG)){// 检查出已运行
				result.put("state", BusinessConstant.PARAMS_START_STATE_ACTIVE);
				...
				result.put("state", BusinessConstant.PARAMS_STOP_STATE_ACTIVE);
			...
			log.debug("Route执行状态检查失败，失败原因: ", e);
	...
	public Map<String, Object> updateSwitch(String dbKey) throws Exception {
		log.debug("Route主备切换， 参数: dbKey: " + dbKey);
		Map<String, Object> result = new HashMap<String, Object>();
		result.put("flag", 1);
		return result;
	}
