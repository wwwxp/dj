 ...
 * 
  ...
  * @Package:      [com.tydic.service.configure.impl]    
  * @ClassName:    [BillingServiceImpl]     
  * @Description:  [Billing业务启停]     
  * @Author:       [Yuanh]     
  * @CreateDate:   [2017-6-13 下午5:08:01]     
  * @UpdateUser:   [Yuanh]     
  * @UpdateDate:   [2017-6-13 下午5:08:01]     
...
public class BillingServiceImpl implements BillingService {
	...
	public Map<String, Object> updateRunAndStopHost(Map<String, Object> param, String dbKey) throws Exception {
		log.debug("启动业务程序, 参数:" + param.toString());
		...
		try {
			...
			for(int i = 0; i < nimbusList.size(); i ++){
				...
				log.debug("Billing执行命令: " + execCmd);
				
				// 执行命令返回结果
				resultStr = cmdUtil.execMsg(execCmd);
				log.debug("Billing执行命令结果: " + resultStr);
			...
			log.debug("启停Billing程序失败， 失败原因: ", e);
	...
	public Map<String, String> updateCheckProgramState(Map<String, Object> param, String dbKey) throws Exception {
		log.debug("Billing状态检查, 参数: " + param.toString() + ", dbKey: " + dbKey);
		...
		try {
			...
			//for(int i = 0;i < nimbusList.size(); i++){
				...
				String busParams = "\""+envHome+"\"";
				execCmd = MessageFormat.format(execCmd, busPath, busShName, busAction, busParams);
				log.debug("Billing状态检查执行命令: " + execCmd);
				
				// 执行命令返回结果
				String resultStr = cmdUtil.execMsg(execCmd);
				log.debug("Billing状态检查命令结果: " + resultStr);
	...
	public Map<String, Object> queryViewConf(Map<String, Object> param, String dbKey) throws Exception {
		log.debug("Billing配置文件信息， 参数: " + param + ", dbKey: " + dbKey);
		...
		try {
			...
			String remotePath = ftpDto.getFtpRootPath() + Constant.CONF + Constant.BUSS_CONF + Constant.RELEASE_DIR
					+ FileTool.exactPath(fileName) + FileTool.exactPath(clusterType) + config_file;
			log.debug("Billing查看配置文件定义， 远程目录: " + remotePath);
			...
			log.error("查询Billing配置文件失败， 失败原因: ", e);
