 ...
 * 
  ...
  * @Description:  [周边业务程序启停实现类]     
  * @Author:       [Yuanh]     
  * @CreateDate:   [2017-6-13 下午5:13:56]     
  * @UpdateUser:   [Yuanh]     
  * @UpdateDate:   [2017-6-13 下午5:13:56]     
...
public class OtherServiceImpl implements OtherService {
	 ...
	 * 日志对象
	 */
	private static Logger log = Logger.getLogger(OtherServiceImpl.class);
	...
	public Map<String, Object> updateRunAndStopHost(Map<String, Object> param, String dbKey) throws Exception {
		log.debug("周边程序启停， 参数: " + param.toString() + ", dbKey: " + dbKey);
		...
		try {
			for (Map<String, Object> paramMap : paramList) {
				...
				log.debug("周边程序启停命令: " + execCmd);
				
				// 执行命令返回结果
				resultStr = cmdUtil.execMsg(execCmd);
				log.debug("周边程序启停结果: " + resultStr);
				...
				returnMap.put("info", "主机【"+StringTool.object2String(hostMap.get("HOST_IP")+"】"+runAndStop+"成功"));
				returnMap.put("reason", resultStr);
				returnMap.put("flag", BusinessConstant.PARAMS_RST_SUCCESS);
			}
		} catch (Exception e) {
			log.error("周边程序启停失败， 失败原因: ", e);
	...
	public Map<String, Object> updateCheckHostState(Map<String, Object> params, String dbKey) throws Exception {
		log.debug("检查周边程序运行状态, 参数: " + params.toString() + ", dbKey: " + dbKey);
		...
		try {
			...
			//业务程序脚本
			String exShName = shName.replace("$P", busVerPath);
			//检查执行命令
			String execCmd = MessageFormat.format(cmd, busPath, exShName, busAction, "");
			log.debug("周边程序状态检查, 执行命令: " + execCmd);
			// 执行命令返回结果
			String resultStr = cmdUtil.execMsg(execCmd);
			log.debug("周边程序状态检查执行命令结果; " + resultStr);
