 ...
 * 
  ...
  * @Package:      [com.tydic.service.configure.impl]    
  * @ClassName:    [RunDiffIPServiceImpl]     
  * @Description:  [区分IP业务程序启停]     
  * @Author:       [Yuanh]     
  * @CreateDate:   [2017-6-28 下午2:49:01]     
  * @UpdateUser:   [Yuanh]     
  * @UpdateDate:   [2017-6-28 下午2:49:01]     
  * @UpdateRemark: [说明本次修改内容]    
  * @Version:      [v1.0]   
  *
 */
@Service
public class RunDiffIPServiceImpl implements RunDiffIPService {
	@Resource
	private CoreService coreService;
	
	private static Logger log = Logger.getLogger(RunDiffIPServiceImpl.class);
	...
	public Map<String, Object> updateRunAndStopHost(Map<String, Object> param, String dbKey) throws BusException {
		log.debug("区分IP程序启停， 业务参数: " + param.toString() + ", dbKey: " + dbKey);
		//返回对象
		...
		if (BlankUtil.isBlank(clusterMap) || BlankUtil.isBlank(clusterMap.get("CLUSTER_DEPLOY_PATH"))) {
			log.error("查询业务集群信息失败，无法启停程序(区分IP业务)，请确保数据正确!");
		...
		for (int i=0; i<paramList.size(); i++) {
			...
			Future<Map<String, Object>> retFuture = pool.submit(new Callable<Map<String,Object>>() {
				...
				public Map<String, Object> call() throws Exception {
					...
					Logger threadLogger = LoggerUtils.getThreadLogger(logPath, logName, "RunDiffIpServiceImpl-" + Thread.currentThread().getName());
					...
					try {
						...
						log.debug("区分IP程序启停结果: " + resultStr);
						...
						if (BusinessConstant.PARAMS_START_FLAG.equals(param.get("flag"))) {
							...
							if (BlankUtil.isBlank(id) || BusinessConstant.PARAMS_UNDEFINED.equals(id)) {
								...
								log.debug("新增启动实例数据成功(区分IP业务)，参数: " + paramMap);
								...
								log.debug("修改启动实例数据成功(区分IP业务)，参数: " + updateMap);
							...
							updateMap.put("ID", StringTool.object2String(paramMap.get("ID")));
							BusParamsHelper.updateProgram(coreService, updateMap, dbKey);
							log.debug("修改停止实例数据成功(区分IP业务)，参数: " + updateMap);
						...
						log.error("区分IP程序启停失败， 主机IP: " + StringTool.object2String(hostMap.get("HOST_IP")), e);
						...
						if (BusinessConstant.PARAMS_START_FLAG.equals(param.get("flag"))) {
							...
							if (BlankUtil.isBlank(id) || BusinessConstant.PARAMS_UNDEFINED.equals(id)) {
								...
								paramMap.put("ID", programTaskId);
								BusParamsHelper.insertTaskProgramWithNewId(coreService, paramMap, dbKey);
								log.debug("启动异常，新增启动实例数据成功(区分IP业务)，参数: " + paramMap);
		...
		try {
			...
			log.info("批量程序处理完成(DIFF)， 总程序数:" + futureList.size());
	...
	public Map<String, Object> updateCheckHostState(Map<String, Object> params, String dbKey) throws BusException {
		log.debug("检查区分IP程序运行状态, 参数: " + params.toString() + ", dbKey: " + dbKey);
		//业务集群ID
		...
		try {
			...
			log.debug("区分IP程序状态检查, 执行命令: " + execCmd);
			// 执行命令返回结果
			String resultStr = cmdUtil.execMsg(execCmd);
			log.debug("区分IP程序状态检查执行命令结果; " + resultStr);
	
			if(resultStr.toLowerCase().indexOf(Constant.FLAG_ERROR) >=0 
					 || resultStr.toLowerCase().indexOf(ResponseObj.FAILED) >=0){
				log.info("区分IP程序状态检查失败：" + resultStr);
				result.put(RST_STATE, BusinessConstant.PARAMS_BUS_4);
				throw new RuntimeException(resultStr);
			}
			//params.put("PROGRAM_NAME", resultMap.get("PROGRAM_NAME"));
			if(resultStr.toLowerCase().contains(BusinessConstant.PARAMS_START_FLAG)){// 检查出已运行
				...
				if(!BusinessConstant.PARAMS_START_STATE_ACTIVE.equals(runState)){// 若当前状态为0或Null,则更新状态
					...
					result.put("info", "【"+params.get("PROGRAM_NAME")+"】当前程序正在运行,已同步数据库");
				}
				result.put("process", resultStr.substring(resultStr.replace(" ","").toLowerCase().indexOf("run") + 3));
			}else {// 检查出未运行
				...
				if(BusinessConstant.PARAMS_START_STATE_ACTIVE.equals(runState)){// 若当前状态为1,则更新状态
					...
					result.put("info", "【"+params.get("PROGRAM_NAME")+"】当前程序未运行,已同步数据库");
				}
			}
			result.put("rstCode", BusinessConstant.PARAMS_RST_SUCCESS);

		} catch (Exception e) {
			log.debug("区分IP执行状态检查失败，失败原因: ", e);

			result.put("state", BusinessConstant.PARAMS_BUS_4);
			result.put("rstCode", BusinessConstant.PARAMS_DO_RST_FAILED);
			result.put("info",  "【"+params.get("PROGRAM_NAME")+"】检查执行失败！");
	...
	public Map<String, Object> updateSwitch(String dbKey) throws Exception {
		log.debug("区分IP主备切换， 参数: dbKey: " + dbKey);
		Map<String, Object> result = new HashMap<String, Object>();
		result.put("flag", 1);
		return result;
	}
